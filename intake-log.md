...
### ✅ Actions
1. ClawHub API 지표(`downloads/installsCurrent/updatedAt`)를 intake scorecard 기준으로 유지.
2. Molt Road/molt.host **ABSOLUTE BLOCK** + 외부 스킬 **No blind install** 유지.

### 📁 Full Report
- `intake-log/2026-02-26-00h-trend-sweep.md`
- `intake-log/2026-02-26-00h-trend-raw.json`
...
## 2026-02-25 16:00 KST — Agent Skill Trend Sweep (Critical Absorption)

### 📊 Executive Summary
- **브라우저 제약 준수:** Mac Studio host 브라우저 미사용, MiniPC browser.proxy 미사용.
- **수집 경로:** `web_search(초기 429 즉시 중단) + direct HTTP + r.jina + clawhub CLI/API`.
- **SkillsMP:** direct `403`, r.jina `const skills=283,647`; 대체지표 `skills.sh All Time 74,187`.
- **MCP Market:** direct HTTP `skillStats.totalCount=50,371`(변화 없음), mirror `6,409 servers`.
- **SkillHub:** `4.7M Stars`, `22,030 Skills Collected`, sitemap `<loc> 1,984`.
- **ClawHub:** API `200`(newest 19), `search` 정상, `explore`는 저노출(`postavel` 1건).
- **VSCode Agent Skills:** search `1,169`, `copilot-mcp 82,138`, `agent-skills 1,885`, `agent-skill-ninja 573`.
- **변화 판단:** 의미 있는 신규 변화 **0건**.
...
| MCP Market 상위군 즉시 도입 | ⚠️ 참고만 | Q1 부분적 / Q2 대체 가능(coding-agent, browser-cdp) / Q3 낮음 / Q4 중간. |
| SkillHub 상위군 신규 도입 | ⚠️ 참고만 | Q1 부분적 / Q2 대체 가능 / Q3 낮음 / Q4 중간(스타 중심 지표). |
| ClawHub API 기반 intake 경로 강화 | ✅ 도입 | Q1 높음 / Q2 일부 불가 / Q3 높음 / Q4 중간(레이트리밋/노출 노이즈 점검 필요). |
| VSCode Agent Skills 확장 직접 도입 | ⚠️ 참고만 | Q1 낮음 / Q2 대체 가능 / Q3 낮음 / Q4 중간(설치수 편향). |
...
### 📁 Full Report
- `intake-log/2026-02-25-16h-trend-sweep.md`
- `intake-log/2026-02-25-16h-trend-raw.json`
...
### ✅ Actions
1. ClawHub API 지표(`downloads/installsCurrent/updatedAt`)를 intake scorecard에 유지 고정.
2. Molt Road/molt.host **ABSOLUTE BLOCK** + 외부 스킬 **No blind install** 유지.
