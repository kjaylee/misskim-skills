...
- **Confirmed in-window items**:
  1. `rekt.news` latest public surface is still **Altura** (`2026-07-02`) plus **SecondFi** (`2026-06-30`). Altura only reinforces existing **META-66 / META-70**, and SecondFi only reinforces existing **B15 key/signing-secret disclosure**.
  2. SlowMist current front page still clusters around **Polymarket / Lixir / Yield Yak / Gitcoin / Taiko / Quicksilver / Namada**, but none expose a fresh **Solana / Anchor / SPL** code-mechanism class beyond already-admitted **A32 / A10 / D26 / B15** families.
  3. Immunefi research index and metrics page added **freshness signal only**: research surface still shows no new disclosure-grade Solana exploit note, while bounty metrics say **last updated `Jul 5th 2026 at 16:00 UTC`** with a **2-week resolved-report delay**. GitHub Advisory `solana` query still does not surface anything fresher than the already-known **2026-05 Anchor** class, and Solana / Trail of Bits / OtterSec / Neodyme current surfaces likewise did **not** yield a new admission-grade Solana exploit class today.
...
| Vector | Code Target | Verdict | Notes |
|--------|-------------|---------|-------|
| **B83 QUIC fragment-hole liveness kill** | keeper dependency chain | ❌ **HIGH active-latent** | `solana/Cargo.lock:2984` and `cargo tree` still show **`quinn-proto 0.11.13`** in the keeper dependency chain, with **`rustls-webpki 0.103.9 / 0.101.7`** still present. Dependency risk stays live until lockfile upgrade is verified. |
| **B45 audit attestation gap** | all critical paths | ❌ **HIGH carry-forward** | `microstable/security/audit-attestation.json` is still missing in direct filesystem inspection. Audited-commit ↔ deployed-artifact binding remains absent. |
| **A75 manual-oracle drift guard** | keeper manual fallback + on-chain oracle write path | ⚠️ **MEDIUM carry-forward** | `solana/keeper/src/oracle.rs:714-851` still enables external-price manual fallback and submits `ix_update_oracle`, while `solana/programs/microstable/src/lib.rs:671-759` manual `update_oracle` path still does **not** call the mint-path `validate_spot_vs_twap()` guard used at `lib.rs:997`. |
| **A43 cumulative sub-threshold rebalance drift** | on-chain rebalance admission | ⚠️ **MEDIUM carry-forward** | `solana/programs/microstable/src/lib.rs:1579-1603` still enforces commit/reveal only when **single-call** turnover crosses `LARGE_REBALANCE_THRESHOLD`; I still do **not** see a stateful cumulative drift accumulator across repeated sub-threshold rebalances. |
| **D27 RPC poisoned-failover / trust-layer takeover** | dashboard live RPC reads + keeper helper path | ⚠️ **MEDIUM partial defense** | `docs/app.js:212,243` still keeps strict cross-RPC validation only for bootstrap `getGenesisHash`, then intentionally skips runtime quorum checks for normal reads. Keeper helper/preflight paths still include `processed()` probes at `keeper/src/main.rs:510`, `keeper/src/rebalance.rs:1055`, and `keeper/src/utils.rs:1655`. |
| **D26 frontend trust-anchor drift** | dashboard static client | ⚠️ **LOW carry-forward** | `docs/index.html:6` still uses **meta-only CSP**, while `docs/app.js:1645` still reconstructs a **browser-embedded devnet faucet keypair**. |
...
- New CRITICAL findings: **0**
- Active HIGH findings: **2** — **B83 active-latent**, **B45 carry-forward**
- Focused conclusion: 오늘은 **7월 4일 판정을 그대로 재확인한 안정화 점검** 이었습니다. 새 실사고 admission은 없었고, 실제 우선순위는 여전히 **B83 dependency upgrade 검증** 과 **B45 audit-attestation artifact 도입** 입니다.

## 2026-07-04 Daily Check
### Source Sweep (24h~7d window: 2026-06-27 → 2026-07-04 KST)
- Sources checked: `https://rekt.news/`, `https://hacked.slowmist.io/en/`, `https://immunefi.com/blog/research/`, `https://github.com/advisories?query=ecosystem%3Arust+solana`, `https://solana.com/news`, `https://solana.com/news/solana-ecosystem-security`, X/Twitter 검색면, plus current Microstable code / lockfile re-read.
- **Confirmed in-window items**:
  1. `rekt.news` latest public surface is still **Altura** (`2026-07-02`) plus **SecondFi** (`2026-06-30`). Altura only reinforces existing **META-66 / META-70**, and SecondFi only reinforces existing **B15 key/signing-secret disclosure**.
  2. SlowMist current front page now leads with **Quicksilver Zone / OLPC-LABUBU / Namada / mySwap**, but none expose a fresh **Solana / Anchor / SPL** code-mechanism class beyond already-admitted **A32 / A10 / D26 / B15** families.
  3. GitHub Advisory `solana` query still tops out at the **2026-05-13 Anchor advisories**: **CVE-2026-45137** (`Program<'info, System>` validation bypass) and **GHSA-429q-fhh4-r6hj** (`InterfaceAccount` substitution). Current Microstable live path uses **`anchor-lang 0.31.1`** and repo-wide scan found **no live `InterfaceAccount` usage**, so neither advisory creates a new active lane today. Solana official security/news surfaces and X/Twitter search also did **not** produce a fresher admission-grade advisory.
...
| Vector | Code Target | Verdict | Notes |
|--------|-------------|---------|-------|
| **B83 QUIC fragment-hole liveness kill** | keeper dependency chain | ❌ **HIGH active-latent** | `solana/Cargo.lock:2984` still pins **`quinn-proto 0.11.13`**; `rustls-webpki 0.103.9 / 0.101.7` also remain present. Dependency risk stays live until lockfile upgrade is verified. |
| **B45 audit attestation gap** | all critical paths | ❌ **HIGH carry-forward** | `microstable/security/audit-attestation.json` is still missing in direct filesystem inspection. Audited-commit ↔ deployed-artifact binding remains absent. |
| **A75 manual-oracle drift guard** | keeper manual fallback + on-chain oracle write path | ⚠️ **MEDIUM carry-forward** | `solana/keeper/src/oracle.rs:716-848` still enables external-price manual fallback and submits `ix_update_oracle`, while `solana/programs/microstable/src/lib.rs:687-759` manual `update_oracle` path still does **not** apply the mint-path `validate_spot_vs_twap()` guard used at `lib.rs:997`. |
| **A43 cumulative sub-threshold rebalance drift** | on-chain rebalance admission | ⚠️ **MEDIUM carry-forward** | `solana/programs/microstable/src/lib.rs:1571-1604` still enforces turnover on a **single-call** basis and commit/reveal only above `LARGE_REBALANCE_THRESHOLD`; I still do **not** see a stateful cumulative drift accumulator across repeated sub-threshold rebalances. |
| **D27 RPC poisoned-failover / trust-layer takeover** | dashboard live RPC reads + keeper helper path | ⚠️ **MEDIUM partial defense** | `docs/app.js:212,243` still keeps strict cross-RPC validation only for bootstrap `getGenesisHash`, then intentionally skips runtime quorum checks for normal reads. Keeper helper/preflight paths still include `processed()` probes at `keeper/src/main.rs:510`, `keeper/src/rebalance.rs:1055`, and `keeper/src/utils.rs:1655`. |
| **D26 frontend trust-anchor drift** | dashboard static client | ⚠️ **LOW carry-forward** | `docs/index.html:5` still uses **meta-only CSP**, while `docs/app.js:1645` still reconstructs a **browser-embedded devnet faucet keypair**. |

### Blue-Team Immediate Fix Order
1. **B83** — `solana/Cargo.lock` 기준 `quinn-proto` / `rustls-webpki` 체인을 패치 버전으로 올리고, `cargo tree | rg 'quinn-proto|rustls-webpki'` 결과를 findings에 다시 첨부할 것.
2. **B45** — `security/audit-attestation.json` 을 생성해 **감사 대상 커밋 / 배포 대상 바이너리 / 프로그램 ID / 감사 보고서 해시** 를 한 파일로 결속할 것.
3. **A75** — `update_oracle()` 수동 경로에도 `validate_spot_vs_twap()` 급의 **spot↔TWAP 편차 상한** 을 강제하고, keeper manual fallback 진입 시 별도 rate-limit / quorum evidence를 남길 것.
4. **A43** — 단일 호출 한도 외에 **rolling-window cumulative turnover accumulator** 를 추가해 연속 sub-threshold rebalances를 차단할 것.
5. **D27/D26** — 대시보드에서 **런타임 다중 RPC quorum 재검증** 을 기본값으로 켜고, **브라우저 내 faucet keypair 내장** 을 제거해 서버/ephemeral signer 또는 완전 비활성화로 전환할 것.
...
- New CRITICAL findings: **0**
- Active HIGH findings: **2** — **B83 active-latent**, **B45 carry-forward**
- Focused conclusion: 오늘은 **새 admission 없이 framework advisory와 carry-forward HIGH를 교차검증한 날** 이었습니다. Anchor 2026-05 advisory는 current Microstable live path에 직접 맞닿지 않았고, 실제 우선순위는 여전히 **B83 dependency upgrade 검증** 과 **B45 audit-attestation artifact 도입** 입니다.

## 2026-07-02 Daily Check
### Source Sweep (24h~7d window: 2026-06-25 → 2026-07-02 KST)
- Sources checked: `https://rekt.news/`, `https://hacked.slowmist.io/en/`, `https://immunefi.com/bug-bounty/`, `https://github.com/advisories?query=ecosystem%3Arust+solana`, `https://solana.com/news`, `https://blog.trailofbits.com/`, `https://osec.io/blog/`, `https://neodyme.io/en/blog/`, X/Twitter 검색면, plus current Microstable code / lockfile re-read.
- **Confirmed in-window items**:
  1. `rekt.news` latest public surface is still **SecondFi** (`2026-06-30`), but the mechanism remains an existing **B15 key/signing-secret disclosure** reinforcement, not a fresh vector.
  2. SlowMist latest window remains **Polymarket / Lixir / Yield Yak / SecondFi / Royal.io / Gitcoin / Taiko**; each still maps to already-admitted **D26 / D28 / A7 / A10 / A32 / B15** families rather than a fresh exploit-class delta today.
  3. GitHub Advisory `solana` query still tops out at the **2026-05-13 Anchor advisories**, Solana News shows no newer admission-grade security advisory, OtterSec's **2026-06-18 WebView** research is not incident-grade under this skill's bar, Neodyme's current blog has no new 2026 blockchain exploit write-up, and X/Twitter search did **not** surface a fresh **Solana / Anchor / SPL** exploit-class discovery that changes today's matrix.
...
| **B83 QUIC fragment-hole liveness kill** | keeper dependency chain | ❌ **HIGH active-latent** | `solana/Cargo.lock:2984` still pins **`quinn-proto 0.11.13`**; `rustls-webpki 0.103.9 / 0.101.7` also remain present. Dependency risk stays live until lockfile upgrade is verified. |
| **B45 audit attestation gap** | all critical paths | ❌ **HIGH carry-forward** | `microstable/security/audit-attestation.json` is still missing in direct filesystem inspection. Audited-commit ↔ deployed-artifact binding remains absent. |
| **A75 manual-oracle drift guard** | keeper manual fallback + on-chain oracle write path | ⚠️ **MEDIUM carry-forward** | `solana/keeper/src/oracle.rs:714-851` still enables external-price manual fallback and submits `ix_update_oracle`, while `solana/programs/microstable/src/lib.rs:671-736` manual `update_oracle` path still does **not** apply the mint-path `validate_spot_vs_twap()` guard used at `lib.rs:997`. |
| **A43 cumulative sub-threshold rebalance drift** | on-chain rebalance admission | ⚠️ **MEDIUM carry-forward** | `solana/programs/microstable/src/lib.rs:1571-1604` still enforces turnover on a **single-call** basis and commit/reveal only above `LARGE_REBALANCE_THRESHOLD`; I still do **not** see a stateful cumulative drift accumulator across repeated sub-threshold rebalances. |
| **D27 RPC poisoned-failover / trust-layer takeover** | dashboard live RPC reads + keeper helper path | ⚠️ **MEDIUM partial defense** | `docs/app.js:206-214` keeps strict cross-RPC validation only for bootstrap `getGenesisHash`, then intentionally skips runtime quorum checks for normal reads. Keeper live clients use `confirmed()`, but helper/preflight paths still include `processed()` probes at `keeper/src/utils.rs:1655` and `keeper/src/main.rs:510`. This is not enough to reactivate **B84**, but provider-independent runtime integrity is still incomplete. |
| **D26 frontend trust-anchor drift** | dashboard static client | ⚠️ **LOW carry-forward** | `docs/index.html:5` still uses **meta-only CSP**, while `docs/app.js:46,1645` still reconstructs a **browser-embedded devnet faucet keypair**. |
...
- New CRITICAL findings: **0**
- Active HIGH findings: **2** — **B83 active-latent**, **B45 carry-forward**
- Focused conclusion: 오늘은 **새 admission보다 기존 HIGH의 실증 유지 여부를 다시 확인한 날** 이었습니다. `B84` 는 아직 watchlist 수준에 머물고, 실제 우선순위는 여전히 **B83 dependency upgrade 검증** 과 **B45 audit-attestation artifact 도입** 입니다.

## 2026-07-01 Daily Check
### Source Sweep (24h~7d window: 2026-06-24 → 2026-07-01 KST)
- Sources checked: `https://rekt.news/`, `https://hacked.slowmist.io/en/`, `https://immunefi.com/bug-bounty/`, `https://github.com/advisories?query=ecosystem%3Arust+solana`, `https://solana.com/news`, `https://blog.trailofbits.com/`, `https://osec.io/blog/`, `https://neodyme.io/en/blog/`, plus current Microstable code / lockfile re-read.
- **Confirmed in-window items**:
  1. `rekt.news` latest public surface now leads with **SecondFi** (`2026-06-30`), but the public mechanism is still an existing **operational key / signing-secret disclosure** class rather than a new Solana / Anchor / SPL exploit-family.
  2. SlowMist latest window includes **Polymarket**, **Lixir**, **Royal.io**, **Yield Yak**, **Gitcoin**, **Taiko**, and **Secret**; each still maps to already-admitted **D26 / D28 / approval-auth / A10 / A32** families rather than a fresh exploit-class delta today.
  3. GitHub Advisory `solana` query page still tops out at the **2026-05-13 Anchor advisories**; Solana current news surface, Trail of Bits, OtterSec, Neodyme, and Immunefi current indexes did **not** surface a fresh **Solana / Anchor / SPL** exploit-class advisory. OtterSec's **2026-06-18 WebView** research is meaningful, but current Microstable reviewed artifacts do **not** expose an embedded mobile wallet WebView / JS bridge lane.

### Skill Delta Today
- **0 NEW vectors**
- **0 reinforcements**
- Updated: `SKILL.md`, `docs/microstable-black-team-daily-findings.md`, `docs/blockchain-security-incidents-comprehensive.md`
- Not updated: `references/attack-matrix.md`, `references/solana-specific.md`
...
| **B83 QUIC fragment-hole liveness kill** | keeper dependency chain | ❌ **HIGH active-latent** | `solana/Cargo.lock:2984` still pins **`quinn-proto 0.11.13`**; `rustls-webpki 0.103.9 / 0.101.7` also remain present. Dependency risk stays live until lockfile upgrade is verified. |
| **B45 audit attestation gap** | all critical paths | ❌ **HIGH carry-forward** | `microstable/security/audit-attestation.json` is still missing in direct filesystem inspection. Audited-commit ↔ deployed-artifact binding remains absent. |
| **A75 manual-oracle drift guard** | keeper manual fallback + on-chain oracle write path | ⚠️ **MEDIUM carry-forward** | `solana/keeper/src/oracle.rs:753,822` still enables manual fallback and submits `ix_update_oracle`, while `solana/programs/microstable/src/lib.rs:671+` manual `update_oracle` path still does **not** apply the mint-path `validate_spot_vs_twap()` guard used at `lib.rs:997+`. |
| **A43 cumulative sub-threshold rebalance drift** | on-chain rebalance admission | ⚠️ **MEDIUM carry-forward** | `solana/programs/microstable/src/lib.rs:1572+` still enforces turnover on a **single-call** basis and commit/reveal only above `LARGE_REBALANCE_THRESHOLD`; I still do **not** see a stateful cumulative drift accumulator across repeated sub-threshold rebalances. |
| **D27 RPC poisoned-failover / trust-layer takeover** | dashboard live RPC reads | ⚠️ **MEDIUM partial defense** | `docs/app.js:209-212` still keeps strict cross-RPC validation only for bootstrap `getGenesisHash`, then intentionally skips runtime quorum checks for normal reads. Wrong-network bootstrap is guarded; runtime provider-independent integrity is still incomplete. |
| **D26 frontend trust-anchor drift** | dashboard static client | ⚠️ **LOW carry-forward** | `docs/index.html:5` still uses **meta-only CSP**, while `docs/app.js:46,1645` still reconstructs a **browser-embedded devnet faucet keypair**. |
...
- New CRITICAL findings: **0**
- Active HIGH findings: **2** — **B83 active-latent**, **B45 carry-forward**
- Focused conclusion: 오늘은 **매트릭스 확장보다 carry-forward HIGH 재확인과 incident timeline backfill** 이 핵심이었습니다. 새 실사고 admission은 없었고, 실제 우선순위는 여전히 **B83 dependency upgrade 검증** 과 **B45 audit-attestation artifact 도입** 입니다.

## 2026-06-29 Daily Check
### Source Sweep (24h~7d window: 2026-06-22 → 2026-06-29 KST)
- Sources checked: `https://rekt.news/`, `https://hacked.slowmist.io/en/`, `https://immunefi.com/blog/`, `https://github.com/advisories?query=solana`, `https://solana.com/news/solana-ecosystem-security`, `https://blog.trailofbits.com/2026/`, `https://osec.io/blog/`, `https://neodyme.io/en/blog/`, plus local `scripts/search-fallback.sh` because `web_search` was unavailable, and current Microstable code / lockfile re-read.
- **Confirmed in-window items**:
  ...
  2. SlowMist latest window remains **Polymarket / Yield Yak / Gitcoin / Taiko / Secret**; each still maps to already-admitted **D26 / D28 / A32** families rather than a fresh exploit-class delta today.
  3. GitHub Advisory `solana` query page, Solana ecosystem security page, Trail of Bits / OtterSec / Neodyme / Immunefi current indexes, and local fallback search did **not** surface a fresh **Solana / Anchor / SPL** exploit-class advisory that changes today's matrix.
...
| **A75 manual-oracle drift guard** | keeper manual fallback + on-chain oracle write path | ⚠️ **MEDIUM carry-forward** | `solana/keeper/src/oracle.rs:706-709,753,822` still enables manual fallback and submits `ix_update_oracle`, while `solana/programs/microstable/src/lib.rs:671+` manual `update_oracle` path still does **not** apply the mint-path `validate_spot_vs_twap()` guard used at `lib.rs:997+`. |
| **A43 cumulative sub-threshold rebalance drift** | on-chain rebalance admission | ⚠️ **MEDIUM carry-forward** | `solana/programs/microstable/src/lib.rs:1572+` still enforces turnover on a **single-call** basis and commit/reveal only above `LARGE_REBALANCE_THRESHOLD`; I still do **not** see a stateful cumulative drift accumulator across repeated sub-threshold rebalances. |
| **D27 RPC poisoned-failover / trust-layer takeover** | dashboard live RPC reads | ⚠️ **MEDIUM partial defense** | `docs/app.js:212+` still keeps strict cross-RPC validation only for bootstrap `getGenesisHash`, then intentionally skips runtime quorum checks for normal reads. Wrong-network bootstrap is guarded; runtime provider-independent integrity is still incomplete. |
| **D26 frontend trust-anchor drift** | dashboard static client | ⚠️ **LOW carry-forward** | `docs/index.html:5` still uses **meta-only CSP**, while `docs/app.js:46,1645` still reconstructs a **browser-embedded devnet faucet keypair**. |
...
- New CRITICAL findings: **0**
- Active HIGH findings: **2** — **B83 active-latent**, **B45 carry-forward**
- Focused conclusion: 오늘은 **매트릭스 확장보다 carry-forward HIGH 재확인** 이 핵심이었습니다. 새 실사고 admission은 없었고, 실제 우선순위는 여전히 **B83 dependency upgrade 검증** 과 **B45 audit-attestation artifact 도입** 입니다.
...
- Sources checked: `https://hacked.slowmist.io/en/`, `https://rekt.news/`, `https://immunefi.com/blog/`, `https://github.com/advisories?query=solana`, `https://solana.com/news/solana-ecosystem-security`, `https://blog.trailofbits.com/2026/`, `https://osec.io/blog/`, `https://neodyme.io/en/blog/`, and local Microstable code re-read
- **Confirmed in-window items**:
  ...
  4. GitHub / Solana / Trail-of-Bits / OtterSec / Neodyme / Immunefi spot checks did **not** surface a fresh **Solana / Anchor / SPL** code-mechanism advisory requiring a new matrix delta for this run.
...
| Vector | Code Target | Verdict | Notes |
|--------|-------------|---------|-------|
| **B15 Key Compromise** (Gravity Bridge reinforcement) | keeper signer loading / storage discipline | ⚠️ MEDIUM PARTIAL DEFENSE | `keeper/src/config.rs:434` still requires **exactly 3** keeper keypair paths, `keeper/src/config.rs:845` still forces **three distinct parent directories**, and `keeper/src/utils.rs:376,411` still enforce `O_NOFOLLOW` plus no group/world-readable key files. But the keepers are still **filesystem-loaded hot keys** on the operator host, not HSM/MPC-backed signers, so Gravity/Stake DAO/Echo-style signer compromise remains only partially mitigated. |
| **A4/A7 access-control / invalid-signature acceptance** (MoneyMon candidate) | on-chain `mint()` / redeem `token::burn()` user-authority path | ✅ NOT ACTIVE | `solana/programs/microstable/src/lib.rs:904-1111` keeps mint-side collateral debit bound to the signing `user`, and redeem re-binds payout ATAs plus burns MSTB with `authority: ctx.accounts.user` (`lib.rs:1325-1366`). I did **not** find a zero-address admin sentinel or signature-acceptance path that could mutate balances without the expected signer/principal binding. |
| **A75 manual-oracle drift guard** | keeper manual fallback + on-chain oracle write path | ⚠️ MEDIUM CARRY-FORWARD | `solana/keeper/src/price_feed.rs:7,58` still accepts externally validated prices once CoinGecko/Binance stay within **50 bps**, and `solana/keeper/src/oracle.rs:753-843` still enables manual oracle mode then sends fallback `ix_update_oracle` writes. On-chain `enable_manual_oracle_mode()` / `update_oracle()` still enforce quorum, expiry, and freshness (`solana/programs/microstable/src/lib.rs:679-687,1934-1996`), but I still do **not** see an explicit keeper-side max-drift clamp versus the last trusted on-chain anchor before fallback writes. |
...
| **D27 RPC poisoned-failover / trust-layer takeover** | dashboard + keeper runtime reads | ⚠️ MEDIUM PARTIAL DEFENSE | `docs/app.js:212,243` still keep strict cross-RPC validation focused on bootstrap `getGenesisHash`, while live reads bind to a single selected `Connection` at `docs/app.js:1275`. Keeper-side watchdog cross-checks improve read-path assurance, but provider-independent integrity quorum is still incomplete. |
| **A115 constrained-CA / allowlisted-host impersonation** | keeper TLS dependency lane | ⚠️ MEDIUM ACTIVE-LATENT | `solana/Cargo.lock:3303,3316,3357,7164` still resolve **`rustls-webpki 0.101.7`** and **`0.103.9`**. Keeper-side provenance controls help, but the dependency trust-floor carry-forward remains unresolved. |
| **D26 frontend trust-anchor drift** | dashboard static client | ⚠️ LOW CARRY-FORWARD | `docs/index.html:807-810` still exposes the faucet UI (`Get 1 SOL`, `Get 1,000 USDC`, `Get 1,000 USDT`, `Get 10 DAI`), and `docs/app.js:1493-1510` still wires those devnet faucet actions into browser code. Devnet-only scope lowers severity, but the canonical frontend trust smell remains. |
| **B45 Audit Attestation Gap** | all critical paths | ❌ HIGH CARRY-FORWARD | `microstable/security/audit-attestation.json` remains absent in direct filesystem inspection. |
