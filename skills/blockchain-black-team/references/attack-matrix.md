...
### A1. Reentrancy
**Historical**: The DAO (2016, $60M), Curve/Vyper (2023, $70M), Cream Finance ($130M), Joe Agent ($45K, 2026-05-28 — low-level BNB send before LP balance update)
...
**2026 reinforcement (Joe Agent, 2026-05-28)**: SlowMist's public incident summary says `_removeLiquidityViaContract` sent BNB via a low-level call **before** decrementing `lpInfo[user].lpAmount`. The attacker reportedly re-entered about 25 times, draining **62.5 BNB + ~1.196M JOE**. Key reusable smell: even a single-function liquidity-removal path becomes drainable when **value transfer precedes LP-share/accounting mutation**.
...
**2026-04-01 reinforcement (LML/USDT Staking Protocol ~$950K — BSC, BlockSec Phalcon)**: **TWAP/Snapshot Reward Oracle + Zero-Address Recipient Path Manipulation**. Mechanism: (1) Attacker executed trades through a path that included a **zero-address recipient** (null address), pushing up the spot price of LML without transferring tokens to a real recipient; (2) the staking protocol's `claim` function calculated rewards based on TWAP or snapshot prices, which were now inflated; (3) attacker invoked `claim` from an address that had previously deposited tokens, capturing the amplified rewards directly. Key insight: **zero-address recipient path** is a novel price-manipulation primitive that bypasses normal DEX reserve accounting — the trade appears to execute but no tokens are actually transferred to a destination, desyncing price-oracle calculations from real liquidity. This pattern extends the BSC Stake Contract class: any protocol that derives reward yield from TWAP/snapshot prices without verifying that the price movement reflects actual token transfers is vulnerable.
**Source**: https://x.com/Phalcon_xyz/status/2039211832947408928 (SlowMist 2026-04-01) | https://hacked.slowmist.io/
...
### A4. Access Control
**Historical**: Ronin ($624M — 5/9 validator keys), Wormhole ($320M — guardian signature bypass), Poly Network ($611M — role verification bypass), Gondi NFT platform (2026-03-10, $230K — Purchase Bundler missing asset owner/borrower verification), Injective Protocol (2026-03-16, $500M at risk — permissionless chain-level account drain), YieldCore-3rd-deal (2026-04-29, ~$398K — vault caller authorization missing), ZetaChain GatewayZEVM / GatewayEVM (2026-04-26, ~$333,868 — unauthenticated gateway call + downstream arbitrary-call execution), TrustedVolumes RFQ proxy (2026-05-07, ~$5.87M — authorized maker/signer pair but third-party resolver debited), Ink Finance Workspace Treasury Proxy (2026-05-11, ~$140K — whitelisted claimer path without entitlement binding), SQ Protocol staking contract (2026-05-12 — hidden hardcoded owner backdoor + authorization-list ownership seize)
...
**2026 reinforcement (YieldCore-3rd-deal, 2026-04-29)**: Public incident summaries attribute the drain to **missing caller permission verification** in the vault path. The important reusable class is not just "forgot onlyOwner" but **helper/vault entrypoints that are assumed to be internal-orchestrator-only yet remain directly callable on-chain**. If the contract can move assets, the caller allowlist must live on the contract boundary itself; workflow assumptions are not access control.
**2026 reinforcement (ZetaChain GatewayZEVM / GatewayEVM, exploit window 2026-04-26)**: Public post-mortem coverage says `GatewayZEVM.call()` accepted arbitrary callers and arbitrary destination/message parameters, which let the attacker emit a syntactically valid cross-chain event that ZetaChain's TSS validators then treated as legitimate. On the destination side, `GatewayEVM.execute()` honored arbitrary external calls, and pre-existing unlimited ERC-20 approvals to the gateway turned the protocol's own gateway into the `transferFrom` caller against victim wallets. **Key pattern**: in cross-chain systems, **message generation is itself a privileged action**. If the source-side gateway can be invoked permissionlessly, downstream relay validation and standing token approvals collapse into one attacker-reachable authority surface.
**2026 reinforcement (Ekubo Protocol extension callback, 2026-05-05)**: SlowMist's public mechanism summary says the vulnerable `IPayer.pay` callback ultimately executed `token.transferFrom(payer, recipient, amount)` with **`payer`, `token`, and `amount` sourced from attacker-controlled lock payload data**. The extension contract did not verify that `payer` was the lock initiator or another explicitly authorized funding source. The attacker therefore converted **old standing ERC-20 approvals** into an arbitrary-drain primitive by naming any previously approved user as `payer` and themselves as `recipient`. **Key pattern**: function authorization or callback reachability is not enough when the function can nominate a third-party funding source. A contract that spends via approvals must separately bind **who is allowed to pay** to the authenticated caller / order / lock context; otherwise `approved spender` silently becomes `arbitrary spender`.
**2026 reinforcement (TrustedVolumes RFQ proxy, 2026-05-07)**: Public trace-based analysis says the attacker first used `registerAllowedOrderSigner(address,bool)` to authorize their own EOA as signer for an attacker-controlled helper contract as maker. The vulnerable RFQ execution path then checked `_allowedOrderSigner[maker][signer]` but pulled tokens from a separate calldata-supplied address (`resolver`) via `transferFrom`. Because the resolver had already approved the proxy, the attacker only needed a valid maker/signer pair they controlled and a victim funding address with standing approvals. **Key pattern**: the contract validated authority relative to one principal and debited assets from another. Authorization must bind the **same principal that is being charged**, or explicitly and cryptographically bind any third-party payer/funder into the signed order.
**2026 reinforcement (Ink Finance Workspace Treasury Proxy, 2026-05-11)**: Public incident coverage says the attacker deployed or controlled an address that passed the workspace controller's whitelisted claimer check, then invoked `claim(claimId)` so the downstream treasury proxy released funds without re-validating what that caller was actually entitled to receive. A flash loan amplified the amount by temporarily satisfying the claim path's balance assumptions. **Key pattern**: `caller is whitelisted` does not prove `caller is entitled to this amount from this treasury source`. Authorization checks on helper/claimer paths must bind both **who may call** and **what value/asset source that caller may consume**.
**2026 reinforcement (SQ Protocol staking contract, 2026-05-12)**: SlowMist's public mechanism summary says the attacker exploited a **hardcoded owner backdoor** left inside the verified staking contract, used the modern `authorizationList` / type-0x4 transaction path to seize effective ownership, then created fake staking claims, redeemed them into USDT, and sold the protocol token. **Key pattern**: a contract can look verified and still hide a privileged ownership path that is reachable through an alternate transaction authorization surface. Security review must enumerate **all ownership-authority mutation paths**, not just the obvious admin function list. **Source**: https://hacked.slowmist.io/
**2026 reinforcement (MEV Bot / JaredFromSubway.eth, 2026-06-20)**: SlowMist's public incident summary says attackers deployed **fake token wrappers and liquidity pools** so an automated MEV execution system granted approvals to **attacker-controlled contracts**, after which the attacker used those standing approvals to pull **WETH, USDC, and USDT** via `transferFrom()`. **Key pattern**: in approval automation, the authorization problem is not only `may this workflow grant an approval?` but also **`to exactly which spender / wrapper / pool graph is that approval being bound?`**. If adversarial discovery or simulation data can nominate the spender, `approved automation` silently degrades into `attacker-chosen spender authority`.
**Code pattern to find**:
```rust
...
**Sources (2026-04-30 / 2026-05-11 / 2026-05-12 / 2026-06-20 reinforcements)**: https://www.panewslab.com/en/articles/019dd6bf-8eba-70b8-9374-b1a150f9da05 | https://www.cryptotimes.io/2026/04/29/how-a-perfect-storm-of-3-bugs-led-to-zetachains-333k-gatewayevm-exploit/ | https://hacked.slowmist.io/ | https://www.darknavy.org/web3/exploits/trustedvolumes-rfq-proxy-drain/ | https://www.cryptotimes.io/2026/05/11/ink-finance-exploited-on-polygon-140k-usdt-drained-in-flash-loan-attack/
...
### B15. Key Compromise
**Historical**: Ronin ($624M), Harmony ($100M), Slope wallet, IoTeX ioTube (2026-02-21, $4.4M), Holdstation DeFAI (2026-02-25, $462K — tentative; root cause under investigation), Volo Vaults (2026-04-22, ~$3.5M), Echo Protocol (2026-05-19, ~11.29 WBTC + 1,000 unbacked eBTC minted), Stake DAO (2026-05-27, ~$91K — deployer key → LayerZero `setPeer()` rewrite → forged mint)
**Mechanism**: Private key stolen from file/memory/HSM → full control of associated accounts. IoTeX ioTube: validator owner private key for the Ethereum-side bridge was compromised; attacker minted two extra tokens on top of the drain. Holdstation: "MFA bypass" in 2 minutes across multi-chain wallet (World Chain, BSC, Berachain, zkSync → ETH → BTC). Both incidents converted stolen funds to ETH/BTC via THORChain. Volo Vaults: protocol disclosed a vault-side security incident and SlowMist classified the public root cause as **Private Key Leakage**, meaning no smart-contract bug was needed once the privileged signer was exposed. Echo Protocol: SlowMist's public incident summary says an **admin private key leak** let the attacker grant themselves **minting rights**, mint **1,000 unbacked eBTC**, post a portion as collateral, borrow **~11.29 WBTC**, and bridge/exfiltrate while the remaining synthetic supply stayed attacker-controlled. Stake DAO: SlowMist's public summary says a **compromised deployer private key** let the attacker change LayerZero v2 `setPeer()` trust configuration, redirect bridge trust to an attacker-controlled peer, forge a cross-chain message, and mint **~5.4T vsdCRV** before swapping out value.
...
**2026 reinforcement (Volo Vaults + Echo Protocol + Stake DAO)**: These incidents reinforce that a protocol can lose treasury assets or create unbacked liabilities without any on-chain invariant failure if a single privileged operator/admin key remains hot or centrally reachable. Echo sharpens the pattern beyond simple treasury withdrawal: once a leaked key can mutate **mint authority / role assignment**, the attacker can fabricate protocol-recognized assets first and only then extract real collateral, turning key compromise into a supply-integrity failure as well as a treasury-loss event. Stake DAO extends the same lesson to bridge control planes: if a leaked deployer/admin key can rewrite **trusted peer / bridge routing configuration** (`setPeer()`), the attacker can promote key theft into a **forged cross-chain mint** rather than a direct wallet drain.
...
**Why audits miss (2026-04 reinforcement)**: 많은 감사는 agent의 system prompt, tool allowlist, on-chain action surface만 본다. 하지만 **router/relay/provider-of-provider** 경로는 보통 SaaS plumbing 또는 infra로 분리되어 감사를 벗어나며, prompt와 tool payload가 그 경계를 지날 때의 **credential visibility / mutation right** 는 거의 검증하지 않는다.
...
**Historical**: BadgerDAO ($120M — Cloudflare Workers compromise injected approval TX), bonk.fun (2026-03-12 — domain hijacking + wallet-draining script injection), Neutrl (2026-03-19 — DNS provider social engineering → domain redirected → Permit2 approval revocation urgent warning), Trust Wallet (2026-04-02 — Discord vanity URL hijack redirected official invite to phishing server), CoW Swap (2026-04-14 — `swap.cow.fi` DNS hijack/front-end domain takeover)
**Mechanism**: Compromise frontend → inject malicious transaction approvals.
**2026 reinforcement (Gitcoin, 2026-06-21)**: rekt's current incident feed says Blockaid detected a front-end attack on Gitcoin's `files.gitcoin.co` subdomain and found malicious **"Eleven drainer"** code on the compromised site. This sharpens D26's **subdomain asset-lane**: attacker does not need the primary landing domain if a trusted static asset/docs/download host under the same brand can still inherit wallet trust and serve drain logic from a canonical-looking origin. The control-plane lesson is not just "protect the main dApp host" but **treat every brand-trusted web origin that can deliver wallet-connected content as production signing surface**.
...
**2026 reinforcement (Trust Wallet, 2026-04-02)**: Trust Wallet's official Discord vanity URL (`discord[.]gg/trustwallet`) was hijacked and redirected users to a phishing server. This broadens D26 beyond website JavaScript injection: **official community entrypoints themselves become attacker-controlled trust anchors**. SlowMist logged the case as Infrastructure Hijacking; Check Point's Discord invite-link research explains the mechanism class — expired temporary invites, lapsed boosted vanity links, and lowercased-code reuse can all silently rebind a previously trusted Discord invite to an attacker server. If a protocol's website, Telegram, or docs embed that invite, the canonical domain amplifies the phishing path even when the website HTML itself is intact.
**2026 reinforcement (CoW Swap, 2026-04-14)**: CoW Swap's main frontend `swap.cow.fi` was hijacked via DNS/registrar control around **14:54 UTC**, forcing the team to pause backend services as a precaution and warn users to **revoke approvals granted after the hijack window**. This sharpens D26's registrar-layer variant: the attacker does not need smart-contract compromise or a lookalike phishing domain — taking over the canonical frontend hostname is enough to inherit protocol trust, trigger approval drain attempts, and force protocol-wide shutdown even when on-chain contracts and backends remain intact.
**2026 reinforcement (Vercel / Context.ai, 2026-04-20)**: Vercel disclosed a breach path starting from a compromised third-party AI tool connection (`Context.ai`) that escalated into internal environments and potentially exposed behind-the-scenes configuration and API keys. For Web3, this matters because many wallet frontends and dApp dashboards are hosted or deployed through the same frontend infrastructure plane. 공격자는 굳이 먼저 브라우저 JS를 바꿀 필요도 없다. **frontend host / deployment steward / attached AI SaaS** 가 이미 environment variables, deployment tokens, backend API credentials에 닿아 있으면, support-surface compromise 자체가 canonical frontend trust를 훔치는 첫 단계가 된다.
...
```
**Defense**: CSP headers (server-level, not just meta tag — meta tag does not block server-injected scripts), SRI hashes, no external scripts, static hosting, domain registrar account MFA, team account isolation (no shared credentials for DNS/hosting), emergency contact/rotation plan for domain compromise. **Permit2 user guidance**: always revoke Permit2 approvals via Revoke.cash after any frontend incident; treat Permit2 revocation as mandatory step in incident response runbooks. **Community-link hardening**: treat Discord/Telegram invite URLs as production assets; avoid reclaimable or ephemeral invite links for official docs, continuously monitor invite ownership/health, and rotate all canonical links immediately on any boost-loss or invite-expiry event.
**Why audits miss (2026-04 reinforcement)**: 스마트콘트랙트 감사와 even many appsec reviews are scoped to **served HTML/JS** and wallet-flow correctness. They usually do **not** own the adjacent frontend-control plane, such as hosting provider OAuth, deployment tokens, environment-variable inheritance, or employee support SaaS attached to that plane. 그래서 canonical domain이 멀쩡해 보여도, 그 뒤의 credential adjacency가 이미 공격자에게 넘어간 경로는 자주 orphan boundary로 남는다.
**Source**: https://rekt.news/ | https://www.cryptonewsz.com/neutrl-front-end-attack-update-urgent-security/ | https://x.com/Neutrl/status/2034445580840370211 | https://hacked.slowmist.io/ | https://www.panewslab.com/en/articles/019d4bcf-a11b-72fe-b597-150de7e31e9f | https://research.checkpoint.com/2025/from-trust-to-threat-hijacked-discord-invites-used-for-multi-stage-malware-delivery/ | https://news.bitcoin.com/cow-protocol-halts-trading-after-frontend-domain-hijack/ | https://www.coindesk.com/tech/2026/04/20/hack-at-vercel-sends-crypto-developers-scrambling-to-lock-down-api-keys
...
### A32 — 2026-04-14 Reinforcement: Hyperbridge ISMP State-Proof Forgery + Token Admin Seizure
**Historical Reinforcement**: Hyperbridge / Polytope Labs token gateway (2026-04-13, Ethereum-side DOT bridge, ~$237K realized after ~1B forged DOT mint; earlier same-day MANTA/CERE probe ~$12K).
**Mechanism**: Attacker deployed a master + helper contract pair and submitted forged state proofs to Hyperbridge's vulnerable `HandlerV1` path. Verification accepted the proof as if it represented a legitimate inbound cross-chain state transition, allowing a malicious `ChangeAssetAdmin` message to flow through `TokenGateway.onAccept()`. That message transferred admin/minter control of the official bridged DOT ERC-20 contract to the attacker, who then minted ~1B DOT and dumped into OdosRouter / Uniswap V4 for ~108.2 ETH.
**Why this reinforces A32**: The attacker did **not** bypass the receiver entirely (so this is not A48). The exploit remained inside the intended relay/proof path; the failure was that proof validation was too weak to bind the message to a real finalized source-chain state. This is the same trust-boundary class as SagaEVM, but with an even more dangerous consequence: bridge proofs can authorize **governance/admin verbs**, not just mint/unlock.
**Why auditors miss the Hyperbridge variant**:
1. proof verifier, message router, token gateway, admin verb handling이 서로 다른 모듈/리포트/팀으로 분리돼 **authority provenance 전체 체인** 을 단일 보안 경계로 보지 않음
2. proof가 형식적으로 valid하다는 사실과, 그 proof가 **누구의 어떤 권한을 운반하는지** 는 다른 질문인데 업계는 전자를 검증하면 후자도 해결됐다고 오인함
3. token bridge는 대개 mint/unlock 위주로 위협 모델링돼 `ChangeAssetAdmin` 같은 control-plane verb를 같은 치명도 도메인으로 묶지 않음
**Audit delta**:
1. Treat `ChangeAssetAdmin`, `setGateway`, `setMinter`, and similar governance/admin payloads as the same critical trust domain as `mint` / `unlock`
2. Proof verification must bind **origin chain + source contract/module + payload type + asset identifier + finalized state root**, not merely a syntactically valid message structure
...
**Why auditors miss the deeper memory-governance layer**:
1. vector DB / chat history / memory store를 성능·제품 기능으로 분류하고 **권한을 운반하는 control plane** 으로 취급하지 않음
2. 메모리 엔트리의 provenance / freshness / namespace / policy scope가 스키마에 강제되지 않아도 “동작한다”는 이유로 설계 통과
3. 단일 세션 프롬프트 방어(B29)만 점검하고, 장기 메모리가 이후 세션의 승인 판단에 영향을 미치는 구조를 별도 감사하지 않음
**Defense**:
...
5. **Governed memory schema**: privileged memory entries must carry `source`, `captured_at`, `trust_scope`, `owner_namespace`, `expires_at` metadata; entries lacking provenance can inform research but cannot authorize action.
...
| META-51 Provenance-Carried Authority Gap (PCAG) — 퍼플팀 2026-04-14 | **핵심 비대칭**: bridge proof, AI memory, vector-store context, artifact manifest처럼 겉보기에는 단순 데이터인 입력이 실제로는 **권한을 운반하는 증거(authority-bearing evidence)** 로 작동한다. 그런데 감사/FV/퍼징은 대개 그 증거가 받아들여진 뒤의 실행 정확성만 검증하고, `source provenance / freshness / ownership / policy scope` 자체를 권한 경계로 취급하지 않는다. **왜 감사가 놓치는가**: ① proof/memory/store를 plumbing·storage·UX 레이어로 분류해 privileged control plane으로 보지 않음. ② 형식 검증과 invariant 테스트가 이미 ingest된 증거를 전제로 출발. ③ 버그바운티는 drain/권한우회 같은 직접 피해에 집중돼 stale/poisoned authoritative context는 저평가. ④ 스키마 설계에서 provenance 필드가 선택 사항으로 남아도 기능 테스트는 통과하기 쉬움. **A32/B43/B52/META-49/META-50과의 구별**: A32·B43·B52는 구체 공격 벡터, META-49는 실행형 설정, META-50은 admission choke. META-51은 **'왜 syntactically valid한 증거가 privileged action이 되는가'** 의 구조적 실패를 규정한다. |
...
| META-69 Provisional-State / Irreversible-Entitlement Gap (PSIEG) — 퍼플팀 2026-05-17 | **핵심 비대칭**: 팀은 `tx seen`, `confirmed`, facilitator ack, off-chain verify-pass 같은 **진행 중 상태** 를 UX상 충분한 신호로 받아들이고, 그 위에서 API 응답·유료 데이터·도구 실행·리소스 접근 같은 **되돌릴 수 없는 entitlement** 를 먼저 열어준다. 그러나 provisional state는 settlement-grade authority가 아니다. x402 연구가 보여주듯 이 경계를 흐리면 grant-before-finality, settlement preemption, replay/idempotency collapse, cache/header leak가 한 묶음으로 터지며 **payment-service correspondence** 자체가 무너진다. **왜 감사가 놓치는가**: ① 스마트컨트랙트 감사는 payment validity를 보고, backend/API 리뷰는 cache/idempotency를 보며, 둘 사이의 **grant threshold** 는 무주공산이 되기 쉽다. ② `confirmed`/receipt/ack를 UX acceptable state로 다루다 보니 그것이 곧 irreversible entitlement threshold라는 오해가 생긴다. ③ 프로토콜 문서가 quote→pay→grant happy path는 길게 설명해도 finality, one-shot binding, cache hygiene, rollback impossibility는 짧거나 생략된다. ④ value가 체인 밖으로 나가는 순간은 되돌리기 어려운데도 팀은 나중 refund/dispute로 메울 수 있다고 착각한다. **META-10/66/B79와의 구별**: META-10은 통합 경계 ownership diffusion, META-66은 assurance plane failure semantics, B79는 구체 exploit primitive다. META-69는 **왜 조직이 intermediate state를 settlement-grade 권한으로 오인해 그런 primitive를 계속 shipping 하는가** 를 다룬다. |
| META-70 Node-Audit / Edge-Semantics Gap (NAESG) — 퍼플팀 2026-05-26 | **핵심 비대칭**: 팀은 contract, keeper, AI agent, bridge verifier, wallet, dashboard 같은 **노드(node)** 를 각각 감사하고 hardening하지만, 실제 공격은 점점 그 사이 **edge semantics** — 누가 signer source를 고를 수 있는가, retry/ACK 순서가 무엇을 의미하는가, 어떤 off-chain evidence가 on-chain mutation을 열 수 있는가, 어떤 agent output이 wallet authority로 승격되는가 — 에서 난다. 최근 Bankr, Butter Bridge, RetoSwap, Mure, WUSD/GLOVE와 Cloudflare Glasswing/Foundry gap signal은 모두 `개별 컴포넌트는 봤지만 경계 간 의미론은 덜 고정됐다` 는 같은 구조를 보여준다. **왜 감사가 놓치는가**: ① audit/FV/fuzz/AI review가 저장소·모듈·함수 단위 coverage에 최적화돼 path/edge별 의미론을 별도 자산으로 잘 다루지 않는다. ② 메시지·ACK·fallback data·verifier source를 plumbing/schema 문제로 낮춰 보고, 실제 authority transition으로 재분류하지 않는다. ③ multi-step path는 각 단계가 individually correct여도 조합 semantics가 틀릴 수 있는데, tool budget과 human review ownership이 그 전체 경로를 끝까지 갖지 않는다. ④ 운영상 false-positive 부담 때문에 runtime cross-check가 bootstrap이나 happy path에만 남고, 위기 때 중요한 edge invariant는 완화되기 쉽다. **META-61/63/69/A125/A127과의 구별**: META-61은 코어 assurance가 주변 plane으로 번지는 후광, META-63은 발견된 속성이 운영 신호가 되는가, META-69는 intermediate state entitlement 문제, A125/A127은 각기 bridge/export·authorization-root의 구체 primitive다. META-70은 그보다 상위에서 **왜 시스템이 node-level로는 잘 보여도 edge-level semantics는 끝까지 소유·검증되지 않는가** 를 규정한다. |
| META-71 Terminal-State / Sentinel Admissibility Gap (TSSAG) — 퍼플팀 2026-06-01 | **핵심 비대칭**: 팀은 `address(0)`, `Pubkey::default()`, `None`, empty root, renounced owner, unset peer 같은 **sentinel/terminal state** 를 “권한 없음” 또는 “수명 종료” 로 인식한다. 그러나 실제 auth logic과 framework wrapper는 그 값을 종종 **비교 가능한 정상값** 으로 남겨 둔다. 그 결과 invalid recovery, untyped fallback, special-case validation collapse가 같은 sentinel로 수렴하면 **실패해야 할 검증이 성공처럼 보이고, 죽은 권한 상태가 다시 privileged truth target으로 재활성화** 된다. **왜 감사가 놓치는가**: ① terminal/unset state를 lifecycle cleanup이나 UX semantics로 보고, 별도의 authorization boundary로 재분류하지 않는다. ② equality check나 typed wrapper를 보는 리뷰어는 비교 대상이 이미 live authority라고 암묵 가정해 `sentinel first-reject` 검증을 생략한다. ③ happy-path 테스트는 valid signer/owner/system-program만 넣고, invalid recovery·default pubkey·renounced owner 같은 **negative terminal states** 를 잘 만들지 않는다. ④ sentinel 값은 framework internals나 config default에 숨어 있어 애플리케이션 코드 리뷰만으로는 collision을 떠올리기 어렵다. **A123/A129/META-54/68과의 구별**: A123과 A129는 각기 typed wrapper / zero-address의 **구체 exploit primitive** 다. META-54는 declared role과 effective authority mismatch, META-68은 legacy surface의 live authority persistence를 다룬다. META-71은 그보다 상위에서 **왜 조직이 ‘비활성/종료 상태는 무해하다’고 믿은 채 그 상태를 여전히 admissible auth value로 남겨 두는가** 를 규정한다. |
...
| A77 rustls-webpki CRL Distribution Point Bypass | RUSTSEC-2026-0049 (2026-03-20, last modified 2026-03-24): rustls-webpki ≤ 0.103.9 — if certificate has >1 distributionPoint, only first is matched against CRL; subsequent DPs ignored. With UnknownStatusPolicy::Allow → revoked cert accepted. With default Deny → false revocation error (connection drop). **2026-04-15 reinforcement**: the same keeper-confirmed version (`rustls-webpki = "0.103.9"`) also remains below the April 15 patch floor for `RUSTSEC-2026-0099` (wildcard DNS name-constraint acceptance) and `RUSTSEC-2026-0098` (URI name-constraint handling; low practical relevance for RPC hostname validation). **2026-04-25 reinforcement**: `RUSTSEC-2026-0104` / `GHSA-82j2-j2ch-gfr8` adds a second CRL-lane failure mode, where a syntactically valid **empty BIT STRING** in `onlySomeReasons` can trigger `bit_string_flags()` panic before CRL signature verification. That turns the same rustls-webpki CRL parsing lane into an **availability kill-switch** when revocation checking is enabled. **Microstable keeper confirmed exposed at dependency level**: Cargo.lock has rustls-webpki = "0.103.9" and "0.101.7" (new combined fix floor: ≥ 0.103.13; `0.103.12` fixed the April 15 name-constraint bugs but not `RUSTSEC-2026-0104`). Attack path: keeper's reqwest/rustls TLS to RPC/oracle APIs. If CRL checking is enabled in a future custom verifier, a malicious or malformed CRL can drop keeper connectivity; if name-constraint or CRL-DP bugs are paired with network position / misissuance, attacker-controlled TLS trust may persist or fail closed at incident time. Severity for keeper: MEDIUM active-latent. Patch: cargo update rustls-webpki to ≥ 0.103.13 via reqwest version bump. (GHSA-pwjx-qhcg-rvj4, GHSA-xgp8-3hg3-c2mh, GHSA-965h-392x-2mh5, GHSA-82j2-j2ch-gfr8) |
...
**2026 reinforcement (Little Boy Plus, 2026-06-17)**: SlowMist's public mechanism summary says a zero-value `transferFrom()` could still reach a vulnerable `_update()` / `_harvest` path and make `mintReward` send fresh **LBP directly into the PancakeSwap pair**. The attacker then drained **USDT** through `swap()` before reserves were re-bound. This extends A91 beyond burn/tax mechanics: **any transfer-time or reward-time side effect that mutates pair-held token balance outside AMM accounting paths is the same reserve-truth failure class**.
...
**2026 reinforcement (LABUBU / OLPC, 2026-06-20)**: SlowMist's public mechanism summary says a previously poisoned `decimalsValue` parameter later caused `_update()` to burn massive **OLPC and LABUBU balances from the pool to the dead address** after only a small transfer. The attacker exploited the resulting stale reserve picture to drain about **$1.1M**. This sharpens A107 in two ways: **(1) delayed-armed config poison can survive long enough to evade correlation with the eventual exploit, and (2) dual-asset pair balance destruction is still just reserve-desync once the AMM accepts stale cache as truth**.
...
**Reinforcement note (2026-06-15)**: Syscoin bridge public postmortem은 이 벡터를 더 직접적으로 보여준다. **malformed proof를 parser가 semantic truth로 오인하면 forged signature 없이도 release lane이 열린다.** 감사가 자주 놓치는 지점은 proof verifier가 canonical valid-proof 사례를 통과하는가가 아니라, **malformed-but-parseable 구조를 hard-fail 시키는지**, 그리고 최종 release가 **source-side burn/lock/reserve conservation** 에 끝까지 결박되는지다. 퍼플팀 문장으로 줄이면: **`proof accepted` ≠ `backing preserved`**.

**Reinforcement update (2026-06-13)**: Syscoin Bridge의 2026-06-08 사고는 A125를 bridge relay / parser semantics 차원에서 다시 강화한다. 공개 메커니즘의 핵심은 **암호학을 깨지 않고도 malformed SPV proof를 relay/parser가 의미론적으로 잘못 받아들이면, validator가 green으로 보는 export path가 실제로는 economically unbacked release를 열 수 있다** 는 점이다. 즉 `proof accepted` 와 `backing preserved` 는 다른 predicate다. 이번 신호는 forged message나 signer compromise가 아니라, **truthful-looking validation lane 자체가 source-side conservation semantics를 충분히 못 박지 못했을 때 벌어지는 무담보 release** 라는 점에서 A125의 순수 강화 사례다.
...
**Published**: 2026-04-14 | **Source**: Purple Team Daily Evolution | **Signals**: Hyperbridge ISMP exploit write-up (2026-04-13), Atlan `AI Agent Memory Governance` (2026-04-02), SwarmSignal AI agent security synthesis updated 2026-04-11, Immunefi Web3 Security Playbook/homepage signals

**Mechanism**: 최신 시스템은 increasingly “데이터처럼 보이는 것”을 **권한의 운반체** 로 사용한다. cross-chain proof는 단순 메시지가 아니라 자산 이동/관리자 변경을 승인하는 근거이고, AI memory/vector-store context는 단순 편의성 캐시가 아니라 다음 세션의 권한 판단·정책 추천·행동 우선순위를 바꾸는 근거다. artifact manifest/attestation도 마찬가지다. 겉으로는 read-only metadata지만 실제로는 “이 증거를 믿고 privileged action을 해도 된다”는 결정을 유도한다.

문제는 업계 assurance가 대부분 **증거가 ingest된 이후** 를 본다는 점이다. 감사는 proof verification 로직, memory access 함수, manifest parser를 본다. 형식 검증은 admissible input이 주어졌다고 가정한다. invariant/fuzz는 이미 로드된 state/context에 대해 실행 중 안전성을 확인한다. 이 과정에서 정작 더 중요한 질문 — **이 입력이 누구의 권한을, 얼마 동안, 어떤 정책 범위 안에서 운반하는가** — 가 구조적으로 누락된다.

즉, syntactic validity와 authority legitimacy가 분리된다.

1. proof가 구조상 valid하지만, 실제 authoritative source에 결박되지 않을 수 있다
2. memory entry가 retrieval상 relevant하지만, provenance/freshness/owner namespace가 없을 수 있다
3. attestation/manifest가 파싱 가능하지만, reviewed artifact와 deployed artifact의 연속성을 보장하지 않을 수 있다

Purple Team 관점에서 이 메타 패턴의 핵심은 **증거가 권한을 운반하는 순간, provenance schema가 곧 access control** 이 된다는 점이다. provenance가 약하면 권한 경계도 약하다.

**Cross-source synthesis (최근 신호의 공통점)**:

- **Hyperbridge (2026-04-13)**: forged state proof가 단순 mint가 아니라 `ChangeAssetAdmin` 까지 운반했다. proof verification은 메시지의 구조를 수용했지만, 그 proof가 어떤 source state와 어떤 authority scope를 운반하는지 충분히 결박하지 못했다.
- **Atlan AI Agent Memory Governance (2026-04-02)**: memory layer에는 provenance, temporal validity, ownership, audit trail이 없으면 governance가 성립하지 않는다고 명시한다. 즉 memory는 storage가 아니라 policy-bearing context plane이다.
- **SwarmSignal update (2026-04-11 fetch)**: prompt injection보다 memory poisoning이 더 위험한 이유는 공격과 실행이 시간적으로 분리돼, 훗날 unrelated session에서 권한 판단을 오염시키기 때문이다. 이는 “현재 세션에 들어온 입력”만 검사하는 보안 모델이 늦다는 뜻이다.
- **Immunefi signals**: `top auditing firms missed` 류의 사례는 단순 감사사 역량 문제가 아니라, 감사 범위가 authority-bearing evidence의 lineage까지 포함하지 못한다는 구조 신호다.

**왜 퍼플팀 관점에서 새 메타 패턴인가**:

| 기존 분류 | 커버하는 것 | 놓치는 것 |
|---|---|---|
| A32 / A113 | 구체적인 bridge proof / authority-meta 경로의 실패 | proof·context·manifest 전반을 authority-bearing evidence로 보는 공통 실패 |
| B43 / B52 | AI memory poisoning의 개별 공격 메커니즘 | memory schema 자체가 governance/control plane이라는 구조 문제 |
| META-49 | executable config가 reviewed source와 deployed artifact를 분리 | 어떤 evidence가 privileged action을 정당화하는지에 대한 provenance 경계 |
| META-50 | admissible set 이전의 admission choke | admission된 evidence가 실제로 정당한 authority를 운반하는지 여부 |

**Microstable architecture implication**:

현재 Microstable은 이 패턴에 대해 비교적 좋은 출발점을 갖는다.

- oracle path는 hardcoded Pyth feed ID + staleness/confidence 검증으로 **source binding** 이 있다
- active cross-chain bridge가 없다
- AI governance/keeper memory layer가 없다

따라서 **active exploit path는 오늘 기준 미확인** 이다. 그러나 향후 다음 확장이 들어오면 META-51은 즉시 활성화된다.

1. manual oracle fallback에서 off-chain validated price를 더 넓게 수용할 때
2. bridge / messaging / external attestation을 mint/redeem/governance 경로에 연결할 때
3. AI assistant가 governance proposal, collateral review, incident triage를 보조할 때

이때 필요한 것은 단순한 “검증 함수”가 아니라, evidence schema 자체에 다음을 강제하는 것이다.

- `source_id`
- `captured_at`
- `expires_at`
- `owner_namespace`
- `trust_scope` (informational / advisory / authoritative / executable)
- cryptographic or human approval binding

**Purple Team verdict**: 2026년의 메타 보안은 더 이상 “입력이 악성인가?”만 묻지 않는다. **그 입력이 권한을 운반하는가, 그리고 그 권한이 provenance에 의해 제한되는가** 를 묻는다. bridge proof, agent memory, artifact manifest를 여전히 단순 데이터로 취급하면, syntactic validity가 privileged action으로 잘못 승격된다. META-51은 이 구조를 “evidence governance failure”로 별도 고정한다.

**Matrix state as of 2026-04-14 (purple daily update)**: **120+ named vectors + META-01~51 + B73~B78**. META-51 added by Purple Team 2026-04-14: **Provenance-Carried Authority Gap (PCAG)**. No new CRITICAL code-level finding from this meta pattern; one latent architecture trust-boundary note documented separately.

### META-52. Metric-Optimized Security Mirage (MOSM)

**Published**: 2026-04-15 | **Source**: Purple Team Daily Evolution | **Signals**: Immunefi March 2026 ecosystem update, Mitchell Amador `The Real Cost of an Onchain Hack: 2024-2025 Update` (fetched 2026-04-15), Hyperbridge exploit coverage (2026-04-13), Nomos Labs fuzz-testing guide (2026), SwarmSignal AI Agent Security synthesis

**Mechanism**: 보안 산업은 지난 2년 동안 분명히 강해졌다. 버그바운티 지급액은 늘고, `live threats prevented` 같은 플랫폼 지표도 증가하고, 형식 검증·퍼징·AI 코드 리뷰는 더 넓은 코드 경로를 더 빠르게 덮는다. 문제는 **이 지표들이 좋아질수록 팀이 “전체 위험도 같이 내려갔다”고 착각하기 쉽다** 는 점이다.

하지만 실제 손실 분포는 다르게 움직인다. 평균적인 버그는 줄어도, power-law tail에 있는 몇 개의 catastrophic failure — proof validation miss, admin authority seizure, signer workflow compromise, artifact continuity break, agent memory/tool confusion — 가 전체 피해 대부분을 계속 결정한다. Purple Team 관점에서 이는 단순한 “tail risk exists” 수준이 아니다. **조직의 측정 체계가 평균을 개선하는 활동에 보상을 주고, 꼬리를 줄이는 활동에는 거의 보상을 주지 않는 구조적 실패** 다.

즉, teams optimize what dashboards can count. Attackers optimize what dashboards do not count.

1. payout amount, audit count, fuzz runs, verified properties는 쉽게 집계된다
2. authority concentration, signer ceremony integrity, attestation freshness, rollback latency, kill-switch coverage는 집계가 어렵다
3. 따라서 예산·리뷰·보상·홍보는 전자 쪽으로 기울고, 후자는 계속 “중요하지만 나중에” 로 밀린다
4. 공격자는 바로 그 나중의 영역을 노린다

**Cross-source synthesis (왜 오늘 이 메타 패턴이 필요한가)**:

- **Immunefi March 2026 update**: $2.7M researcher payouts, 247 live threats prevented, median hack downtrend 같은 긍정 지표가 강하게 전면 배치된다. 이는 분명한 진전이지만, 무엇이 측정되고 무엇이 빠지는지도 함께 보여준다.
- **Amador 2024-2025 hack impact update**: median theft는 줄었지만, top 5 hacks가 stolen funds의 62%, top 10이 73%를 차지한다. 즉, 보이는 평균은 개선돼도 catastrophic tail은 더 지배적일 수 있다.
- **Hyperbridge (2026-04-13)**: 단 하나의 proof-validation failure가 `ChangeAssetAdmin` 으로 이어져 admin/minter seizure를 열었다. 이는 “취약점 수”나 “fuzz coverage”가 아니라 **blast radius가 큰 control-plane miss** 하나가 문제였음을 보여준다.
- **Nomos fuzz-testing guide**: Echidna/Medusa/Foundry 조합은 분명 강력하다. 그러나 그 강함은 어디까지나 모델링된 상태·입력·불변식 범위 안의 correctness 향상이다. provenance, signer ceremony, artifact continuity, emergency override latency는 여전히 테스트 밖일 수 있다.
- **SwarmSignal AI Agent Security**: prompt injection·memory poisoning·tool misuse는 전형적인 low-frequency/high-blast control-plane failure다. agent security도 결국 “평균적인 prompt hygiene”보다 **권한 있는 context와 tool boundary** 가 더 중요해진다.

**왜 퍼플팀 관점에서 새 메타 패턴인가**:

| 기존 분류 | 설명하는 것 | META-52가 추가로 설명하는 것 |
|---|---|---|
| META-48 OCHTG | 공격 표면이 인간/오프체인까지 확장 | 그런데도 왜 업계 예산·지표는 계속 온체인 correctness 쪽으로 쏠리는가 |
| META-49 ECTD | 설정 파일이 실행면이 되는 구조 | 왜 이런 non-code control plane이 대시보드에서 거의 안 보이는가 |
| META-50 ASG | admission choke를 보안 경계로 봐야 함 | 왜 팀이 admission/rollback KPI를 성능 이슈로만 취급하는가 |
| META-51 PCAG | authority-bearing evidence의 provenance 문제 | 왜 provenance schema 구축이 bounty/FV/audit 우선순위에서 밀리는가 |

**Microstable architecture implication**:

오늘 기준 active exploit path를 새로 입증한 것은 아니다. 다만 Microstable도 이 메타 패턴에서 완전히 자유롭지 않다.

- 좋은 신호: hardcoded Pyth binding, stable collateral scope, bridge 부재, AI governance memory 부재
- 나쁜 신호: **B45 artifact attestation gap** 은 바로 “대시보드에 잘 안 잡히지만 blast radius가 큰” 전형적 통제면 리스크다
- carry-forward인 **A43 admission drift**, **META-51 future evidence schema risk** 역시 대부분의 green build/test/security metrics에서 과소표시될 수 있다

따라서 Purple Team 기준의 방어는 단순히 “tests passed / no new CVE / no new bounty hit” 로 끝나면 안 된다. 아래처럼 **tail-risk KPI** 를 별도로 가져가야 한다.

- `max_single_action_blast_radius`
- `attestation_freshness`
- `time_to_revoke_or_override`
- `authority_concentration_map`
- `evidence_provenance_coverage`

**Purple Team verdict**: 2026년 보안의 착시는 “보이는 지표가 좋아지니 전체가 더 안전해졌다”는 믿음에서 나온다. 실제로는 **측정 가능한 중간층 위험은 줄고, 측정되지 않는 control-plane tail risk는 그대로이거나 더 커질 수 있다**. META-52는 이 구조를 **Metric-Optimized Security Mirage** 로 고정한다.

**Matrix state as of 2026-04-15 (purple daily update)**: **121+ named vectors + META-01~52 + B73~B78 = 174+ total entries**. META-52 added by Purple Team 2026-04-15: **Metric-Optimized Security Mirage (MOSM)**. No new CRITICAL code-level finding from this meta pattern; one latent architecture-level measurement/blast-radius note documented separately.

### META-53. Runbook-to-Actuator Binding Gap (RABG)

**Published**: 2026-04-17 | **Source**: Purple Team Daily Evolution | **Signals**: CybersecurityDive/Sygnia IR readiness survey (2026-04-13), Immunefi `How fragmented security enabled the $100m Balancer exploit`, Foundry releases (2026-04-15/16), Hyperbridge exploit coverage (2026-04-13), Unit42/Vertex AI `Double Agent` coverage (2026-04-14)

**Mechanism**: 최근 보안 스택은 detection, simulation, correctness verification에서는 빠르게 강해진다. monitoring이 더 빨리 이상을 잡고, invariant/fuzz/FV가 더 많은 실행 경로를 덮고, incident plan/wargame도 거의 모든 조직이 갖고 있다고 말한다. 문제는 공격을 실제로 멈추는 **마지막 한 단계** — `pause`, `redeem-only`, `mint_limit=0`, `manual_oracle_mode`, `freeze signer`, `switch RPC`, `revoke role` — 가 종종 runbook의 문장으로만 존재한다는 점이다.

즉, 아래 네 층이 서로 단절된다.

1. **Detection**: 이상 징후를 봄
2. **Decision**: 이것이 incident라고 판단함
3. **Actuation**: 누가 어떤 키/권한으로 어떤 명령을 날려 containment를 실행함
4. **Recovery**: 복구·해제·사후 검증을 함

보안 조직은 1·2·4를 문서화하지만, 3을 자주 암묵지로 남긴다. 그 결과 “우리는 대응 계획이 있다”는 말은 참이어도, **공격 템포 안에 실제로 방어 명령이 체인에 올라간다** 는 보장은 없다.

이 메타 패턴의 핵심은 **control exists ≠ actuator is bound** 다.

- pause 함수가 코드에 있어도 signer ceremony가 정해져 있지 않을 수 있다
- keeper quorum이 있어도 auto/manual toggle이 꺼져 있어 human coordination이 병목일 수 있다
- runbook에 `mint_limit=0` 이 적혀 있어도 실제 명령어/계정/권한이 연결되지 않을 수 있다
- dashboard alert가 울려도 board/legal/comms/ops 중 누가 final authority인지 불명확할 수 있다

이때 공격자는 새 0day가 없어도 충분하다. **containment보다 빠르게 움직이기만 하면 된다.**

**Cross-source synthesis (최근 신호의 공통점)**:

- **Sygnia survey / CybersecurityDive (2026-04-13)**: 조직의 99%가 formal incident response plan을 갖고도, 73%는 다음 공격에 충분히 준비되지 않았다고 답했다. 주요 병목은 stakeholder coordination, executive/board involvement 부족, legal/communications delay였다. 즉 문제는 “계획 없음”이 아니라 **actuation latency** 다.
- **Immunefi Balancer meta article**: Balancer 사례를 단일 버그가 아니라 controls operating in isolation 문제로 설명한다. 특히 pause/containment가 설계·모니터링·운영과 분리되면, subtle accounting issue가 systemic failure로 승격될 수 있음을 보여준다.
- **Foundry 2026-04-15/16 releases**: `fail_on_assert` 같은 invariant 캠페인 강화는 execution correctness를 더 잘 잡게 하지만, emergency command path와 signer choreography가 실제로 발사되는지는 다루지 않는다.
- **Hyperbridge (2026-04-13)**: proof-based security narrative가 깨진 뒤 결국 남는 질문은 “얼마나 빨리 멈출 수 있나”였다. proof 검증 miss 이후 containment actuator가 늦으면, architecture narrative의 우월성은 실전 가치가 급락한다.
- **Vertex AI `Double Agent` (2026-04-14)**: overprivileged default agent scopes는 권한 자체의 문제지만, 동시에 incident 발생 시 BYOSA/least-privilege로 바로 전환할 **operational actuator path** 가 준비돼 있지 않으면 damage window가 길어진다.
- **OWASP Agentic Skills Incident Response Playbook (2026-04-28 fetched)**: 최근 공개 IR 문서는 이제 단순 체크리스트가 아니라 severity별 SLA, 정확한 containment verb, user-notification 순서까지 제시한다. 이 신호는 **"runbook이 있다" 와 "발사 가능한 actuator artifact가 미리 결박돼 있다" 는 전혀 다른 상태** 임을 더 선명하게 보여준다.

**왜 퍼플팀 관점에서 새 메타 패턴인가**:

| 기존 분류 | 설명하는 것 | META-53이 추가로 설명하는 것 |
|---|---|---|
| META-48 OCHTG | 인간/오프체인 trust gap | incident 때 실제 containment command를 누가 어떻게 실행하는지 |
| META-50 ASG | admission choke와 liveness 경계 | incident 판정 후 방어 actuator까지의 command-binding 문제 |
| META-51 PCAG | authority-bearing evidence provenance | actuator를 발사하는 권한·키 ceremony와 latency budget |
| META-52 MOSM | 왜 조직이 blind spot에 과소투자하는가 | 그 blind spot이 바로 "마지막 1단계 actuation" 임을 명시 |

**Microstable architecture implication**:

Microstable은 containment primitive 자체는 갖고 있다.

- on-chain `emergency_shutdown` 는 **2-of-3 keeper quorum** 으로 실행된다
- 실행 시 `protocol.emergency_shutdown = true`, `circuit.mint_rate_limit = 0`, `optimizer_enabled = false` 가 적용된다
- keeper monitor는 emergency condition을 감지해 shutdown instruction을 보낼 수 있는 코드 경로를 갖는다

그러나 오늘 기준 구조적 빈틈도 분명하다.

- `docs/ops-runbook.md` 는 playbook 레벨에서 `mint_limit=0` 를 지시하지만, **정확히 어떤 instruction / 어떤 signer ceremony / 어떤 성공 확인 절차** 로 연결되는지 적지 않는다
- `solana/keeper/config.devnet.json` 은 `auto_emergency_shutdown = false` 로 두고 있어, containment가 기본적으로 **manual coordination** 에 의존한다
- 따라서 incident detection과 on-chain freeze 사이의 마지막 연결선은 여전히 사람·절차·가용성에 기대고 있다

이것은 오늘 당장 exploitable code bug라는 뜻은 아니다. 다만 Purple Team 관점에서는 **문서에 있는 control이 실제 공격 속도 안에 발사되는가** 가 별도 검증 대상이라는 뜻이다.

**Purple Team verdict**: 2026년의 방어 실패는 increasingly “취약점이 없었는데도 졌다”는 형태를 띤다. 이유는 간단하다. detection과 correctness는 개선됐지만, **containment actuator가 계획·권한·명령·시간예산에 강하게 결박되지 않았기 때문** 이다. META-53은 이 구조를 **Runbook-to-Actuator Binding Gap** 으로 고정한다.

**Reinforcement note (2026-06-09)**: Morgan Lewis의 2026 incident-response guidance와 SlowMist의 **Syscoin Bridge / Gnosis Pay / Ambient Finance** current-incident 조합은 같은 사실을 다시 보여준다. 팀은 종종 `pause exists`, `delay module exists`, `monitor exists`, `runbook exists` 를 곧바로 방어 capability로 읽지만, 실제 승패는 **의심 신호가 떴을 때 어떤 off-band 연락선과 어떤 signer ceremony로 어떤 containment verb를 몇 분 안에 발사하느냐** 에서 갈린다. 특히 root cause가 아직 완전히 닫히지 않은 상태에서라도 `freeze / revoke / manual mode / spend stop` 같은 actuator를 선제 실행할 수 있어야 하는데, 이 경로가 문서·채팅·사람 기억에만 남아 있으면 green monitoring이나 policy text는 false assurance가 된다. 퍼플팀 관점에서 오늘 신호는 새 admission이 아니라, **control 존재 ≠ bound actuator availability** 라는 META-53의 실전 강화다.

**Matrix state as of 2026-04-17 (purple daily update)**: **121+ named vectors + META-01~53 + B73~B78 = 175+ total entries**. META-53 added by Purple Team 2026-04-17: **Runbook-to-Actuator Binding Gap (RABG)**. No new CRITICAL code-level finding from this meta pattern; one latent architecture-level containment-path note documented separately.

### META-54. Declared-Role / Effective-Authority Gap (DREAG)

**Published**: 2026-04-18 | **Source**: Purple Team Daily Evolution | **Signals**: CybersecurityDive/Sygnia IR readiness survey (2026-04-13), Immunefi `How fragmented security enabled the $100m Balancer exploit`, Foundry releases (2026-04-15/16), Hyperbridge exploit coverage (2026-04-13), Unit42/Vertex AI `Double Agent` coverage (2026-04-14)

**Mechanism**: 팀은 component를 이름·소유조직·UX 역할로 분류한다. `dashboard` 는 read-only, `agent` 는 assistant, `proof` 는 data, `runbook` 는 문서라고 생각한다. 그러나 실제 실패는 **declared role** 이 아니라 **effective authority** 에서 난다. 표면이 어떻게 불리느냐와 무관하게, signer를 들고 있거나, admin verb를 운반하거나, project-wide service-account scope를 물고 있으면 그것은 이미 privileged control plane이다.

이 메타 패턴이 겨냥하는 착시는 다음과 같다.

- “이건 대시보드니까 읽기 전용일 것”
- “이건 proof layer니까 데이터 검증만 하면 된다”
- “이건 assistant agent니까 실행권한은 없을 것”
- “이건 devnet/demo 코드니까 production-grade secret hygiene를 적용하지 않아도 된다”

문제는 공격자가 이 라벨을 믿지 않는다는 점이다. 공격자는 **실제 권한이 있는 곳** 으로 간다.

**Cross-source synthesis (최근 신호의 공통점)**:

- **Hyperbridge (2026-04-13)**: proof-based security narrative 아래 있던 메시지 경로가 단순 데이터가 아니라 `ChangeAssetAdmin` 급 admin authority를 운반했다. 즉 `proof` 로 보인 surface가 실제로는 privileged action path였다.
- **Vertex AI `Double Agent` (2026-04-14)**: assistant-like agent surface가 과도한 default scope를 물고 있어, 단일 에이전트 compromise가 project-wide storage/artifact plane으로 확장됐다. 즉 `agent` 라는 제품 라벨이 권한 범위를 축소해 보여줬다.
- **Immunefi Balancer meta article**: exploitability는 핵심 math bug 하나만이 아니라, internal balance / pause / operational containment 같은 “주변 control surface” 들의 연결 방식에 의해 결정됐다. 즉 주연 계약이 아닌 보조 surface도 실제로는 공격 성공 여부를 좌우하는 authority-adjacent plane이다.
- **Sygnia survey / CybersecurityDive (2026-04-13)**: 조직은 IR 문서를 갖고 있지만 실제 책임/권한/행동 경로는 흐린 경우가 많다. 이는 `runbook=document` 라벨이 실제 decision authority mapping을 가리는 구조와 맞닿아 있다.
- **Foundry releases (2026-04-15/16)**: invariant/assert correctness는 강화되지만, “이 supposedly read-only surface가 애초에 signer를 품고 있는가” 는 묻지 않는다. correctness tooling이 permission topology audit를 대체할 수 없다는 뜻이다.

**왜 퍼플팀 관점에서 새 메타 패턴인가**:

| 기존 분류 | 설명하는 것 | META-54가 추가로 설명하는 것 |
|---|---|---|
| META-51 PCAG | authority-bearing evidence의 provenance | evidence가 아니라 **surface 자체가 role label과 다르게 권한을 품는 구조** |
| META-53 RABG | detection→actuation 마지막 연결선 | actuator 이전에 **어떤 표면이 privileged 인지 잘못 분류되는 문제** |
| D26/B29/B36 | 개별 UI·agent·opsec 벡터 | 왜 팀이 그런 표면을 애초에 privileged review 대상으로 올리지 않는가 |

**Microstable architecture implication**:

Microstable dashboard는 스스로를 다음과 같이 소개한다.

- `docs/index.html`: **"Browser-only, zero backend, direct Solana devnet polling."**

이 설명만 보면 이 표면은 observability/read-only plane처럼 보인다. 그러나 실제 구현은 다르다.

- `docs/app.js`: `FAUCET_CONFIG.instructionAvailable = true`
- `docs/app.js`: browser bundle 안에 `faucetKeypair` 64-byte secret이 직접 포함됨

즉 오늘 확인된 문제는 단순히 “devnet faucet secret이 노출됐다” 를 넘는다. 더 큰 구조는 **read-only observability plane이 조용히 write-capable execution plane으로 바뀌었다** 는 점이다. 이 상태에서는 UI hardening(CSP, domain, bundle integrity)만으로 충분하지 않다. 먼저 **왜 대시보드가 signer를 가져도 된다고 생각했는가** 를 물어야 한다.
...
**Published**: 2026-04-25 | **Source**: Purple Team Daily Evolution | **Signals**: Chainalysis `Inside the KelpDAO Bridge Exploit` (2026-04-24 fetched, incident 2026-04-18), Immunefi/Base audit competition scope (2026-04-21), CoinDesk `Arbitrum freezes $71 million in ether tied to Kelp DAO exploit` (2026-04-21), CoinDesk `AI agents are set to power crypto payments, but a hidden flaw could expose wallets` (2026-04-13, in-window), CoinDesk `Hack at Vercel sends crypto developers scrambling to lock down API keys` (2026-04-20), recent Foundry release checks (2026-04-18~23), invariant/FV channel recheck

**Mechanism**: 업계는 정상 경로의 correctness를 빠르게 강화한다. audit competition은 main path code review를 촘촘히 하고, FV/invariant/fuzz는 명시된 상태 전이를 더 많이 덮고, 운영팀은 incident 때 쓸 emergency power를 문서에 적는다. 그런데 사고가 실제로 터지는 순간, decisive control plane은 곧바로 다른 곳으로 이동한다.

- invalid proof는 `dispute` / `blacklist` / `retire proposal` 로 처리된다.
- verifier/RPC compromise는 `manual restart` / `failover` / `pause` 로 처리된다.
- stolen funds는 `security council freeze` / intermediary wallet / governance recovery path로 이동한다.
- oracle stress나 liveness failure는 `manual oracle mode`, `redeem-only`, `emergency shutdown` 으로 넘어간다.

즉 시스템은 nominal protocol에서 **exception protocol** 로 전환된다. 문제는 이 예외 프로토콜이 대개 정상 경로만큼의 assurance를 받지 못한다는 점이다.

- 정상 경로는 spec, invariant, audit scope, bounty economics, FV/fuzz coverage를 가진다.
- 예외 경로는 종종 "필요하면 개입한다" 는 가정, governance emergency power, downgrade clause, incident runbook 부록으로만 남는다.

결과적으로 **가장 권한이 강하고 blast radius가 큰 순간에, 가장 얕게 검증된 경로가 활성화** 된다.

**Cross-source synthesis (최근 신호의 공통점)**:

- **Base / Immunefi scope (2026-04-21)**: invalid proposal은 dispute/blacklist/retire 가능성이 있으면 downgrade 근거가 되고, service/program이 manual restart with different configuration 될 수 있으면 valid bug도 낮춰 볼 수 있다고 적는다. 이는 exception path가 이미 실질적 safety claim으로 쓰이지만, 그 path 자체가 formal assurance 대상으로 다뤄지지는 않음을 보여준다.
- **Arbitrum Security Council freeze (2026-04-21)**: council은 실제로 30,766 ETH를 intermediary wallet로 이동시켰다. 이것은 단순한 runbook 존재가 아니라, **예외 경로가 자산 소유권과 회수 가능성을 직접 재배열하는 별도 프로토콜** 임을 뜻한다.
- **KelpDAO post-mortem (Chainalysis, fetched 2026-04-24)**: 온체인 calldata는 정상처럼 보였고, 손실 상한과 후속 회수 가능성은 결국 pause, blacklist, external security-council action에 의해 결정됐다. 즉 crisis outcome은 nominal bridge correctness만으로 설명되지 않는다.
- **AI router / Vercel signals (2026-04-13, 2026-04-20)**: 이 둘은 이미 B29/D26/META-54/58을 강화했지만, 동시에 중요한 보조 증거를 준다. 공격은 주로 nominal path 밖의 transport/support plane에서 시작되고, 방어 역시 code path보다 exception handling과 credential rotation path로 넘어간다. 그런데 이 recovery/containment lane은 정상 경로처럼 잘 모델링되지 않는다.
- **Foundry / invariant / FV 채널 재점검**: 최근 신호는 정상 경로 correctness와 stateful sequence coverage를 더 잘 다루지만, `blacklist`, `manual restart`, `freeze`, `manual oracle write`, `recovery socialization` 같은 exception semantics를 1급 대상으로 다루는 공개 delta는 약했다. 즉 오늘 신규 메타는 "도구가 나쁘다" 가 아니라, **도구가 주로 nominal lane에 최적화돼 있다** 는 비대칭이다.

**왜 퍼플팀 관점에서 새 메타 패턴인가**:

| 기존 분류 | 설명하는 것 | META-59가 추가로 설명하는 것 |
|---|---|---|
| META-53 RABG | emergency actuator를 제때 발사할 수 있는가 | 발사된 뒤 **그 예외 경로가 어떤 안전성/공정성/잔고 보전 규칙을 보장하는가** 를 다룬다 |
| META-54 DREAG | 누가 실제 권한을 가지는가 | 권한 보유는 설명하지만, **고권한 예외 경로가 얼마나 깊게 검증됐는지** 는 다루지 않는다 |
| META-58 DSCRG | default path와 scope carve-out의 ownership 부재 | ownership이 있어도, **예외 프로토콜 자체의 assurance 부재** 는 별도 문제다 |
| META-25 FVSC / META-50 ASG | 명세 불완전성, admissibility 이전 경계 | 정상 경로의 spec/admission 문제를 넘어, **위기 시 전환되는 alternate protocol semantics** 의 얕은 검증을 다룬다 |

**왜 감사가 놓치는가**:

1. **nominal-path bias**: audit/FV/fuzz/competition은 코드의 정상 상태 전이와 명시된 함수 경로를 우선 다룬다.
2. **governance/IR appendix treatment**: blacklist, dispute, freeze, manual restart, loss socialization은 protocol semantics가 아니라 운영/거버넌스 부록처럼 분류된다.
3. **backstop-as-proof 착시**: "필요하면 사람이 개입할 수 있다" 는 사실이 곧 safety guarantee처럼 취급된다.
4. **recovery fairness blind spot**: 예외 경로가 어떤 사용자에게 어떤 순서로 손실/회수를 배분하는지, 언제 정상 경로로 복귀하는지, 복귀 전에 어떤 상태 화해(reconciliation)가 필요한지는 잘 모델링되지 않는다.

**Microstable architecture implication**:

현재 Microstable은 Arbitrum-style council이나 bridge recovery wallet은 없다. 하지만 exception lane 자체는 이미 존재한다.

- `emergency_shutdown`
- degraded safe mode
- `manual oracle mode`
- 향후 governance/parameter recovery 경로

Blue v14/v15는 degraded에서도 write path를 막고 auto emergency shutdown 방어선을 유지하는 쪽으로 개선했다. 이것은 중요하다. 다만 오늘 시그널은 그 다음 질문을 요구한다.

- manual oracle mode에서 허용되는 **price deviation / source precedence / expiry / rollback 조건** 이 충분히 명세됐는가
- emergency shutdown 이후 **re-enable 조건 / signer ceremony / 잔고·가격 상태 reconciliation** 이 고정돼 있는가
- exception mode일 때 dashboard, keeper, operator가 **어떤 semantic guarantees** 를 사용자에게 제공하는지 문서화돼 있는가

현재 공개 문서 기준으로는 이 예외 경로들이 하나의 **exception-lane invariant set** 으로 묶여 있지 않다. 따라서 today active exploit은 아니지만, MEDIUM latent architecture watch로 승격할 만하다.

**Minimum defense delta**:

1. nominal path와 별도로 **exception-path spec** 를 작성: allowed verbs, signer set, latency budget, exit criteria, reconciliation rule
2. `manual oracle mode`, `redeem-only`, `emergency_shutdown`, `freeze/recovery` 각각에 대해 **preserved invariants** 명시: max drift, no preferential unlock, user-balance conservation, recovery ordering
3. FV/invariant/fuzz 또는 최소한 table-top simulation을 **exception transitions** 에도 적용
4. `문제 생기면 수동 개입` 문장을 risk downgrade 근거로 쓰기 전에, 그 개입이 **무엇을 보장하지 못하는지** 까지 적시
5. emergency path 종료 후 정상 복귀까지의 **state reconciliation checklist** 를 운영 runbook이 아니라 protocol artifact로 승격

**Purple Team verdict**: 많은 팀은 위기 대응 권한이 있다는 사실을 안전성의 일부로 계산하지만, 실제로는 그 순간 시스템이 다른 프로토콜로 바뀐다. META-59는 이 구조, 즉 **정상 경로는 정교하게 검증되고 예외 경로는 가정으로 남는 비대칭** 을 **Nominal-Path / Exception-Path Assurance Asymmetry** 로 고정한다.

**Matrix state as of 2026-04-25 (purple daily update)**: **123+ named vectors + META-01~59 + B73~B78 = 183+ total entries**. META-59 added by Purple Team 2026-04-25: **Nominal-Path / Exception-Path Assurance Asymmetry (NPEAA)**. Base scope의 downgrade assumptions, Arbitrum의 emergency freeze, KelpDAO recovery reality를 하나의 "exception protocol under-assurance" 패턴으로 상위 구조화한다.

### META-60. Recoverability-Collateralized Security Gap (RCSG)

**Published**: 2026-04-26 | **Source**: Purple Team Daily Evolution | **Signals**: Immunefi/Base audit competition scope (2026-04-21), CoinDesk `Kelp DAO claims LayerZero’s default settings...` (2026-04-20), CoinDesk `Hack at Vercel sends crypto developers scrambling to lock down API keys` (2026-04-20), CoinDesk `Arbitrum freezes $71 million in ether tied to Kelp DAO exploit` (2026-04-21), CoinDesk `Inside the $71 million freeze on Arbitrum...` (2026-04-23), Chainalysis `Inside the KelpDAO Bridge Exploit` (2026-04-25 fetched, incident 2026-04-18), recent Foundry release checks (2026-04-18~23)

**Mechanism**: META-59가 "예외 경로가 정상 경로만큼 보장되지 않는다" 는 점을 고정했다면, META-60은 그보다 한 단계 앞선 조직적 왜곡을 겨냥한다. 많은 팀은 **회수 가능성(recoverability)** 을 실제 보안 통제처럼 취급한다.

- `manual restart with different configurations` 가 가능하면 severity를 낮춘다.
- `dispute / blacklist / retire proposal` 가능성이 있으면 valid report를 downgrade한다.
- incident 후 `security council freeze`, `credential rotation`, `recovery fund`, `loss socialization` 이 작동하면 사후적으로 “생각보다 덜 위험했다” 고 해석한다.

즉 **예방(prevention)과 회수(recovery)를 같은 보안 예산 안에서 혼동** 한다. 그 결과 privilege/control-plane compromise, support-surface credential adjacency, provider default dependency, operator key path 같은 경계는 실제 blast radius에 비해 낮은 severity와 낮은 보상, 낮은 리뷰 강도를 받는다.

**Cross-source synthesis (최근 신호의 공통점)**:

- **Immunefi / Base scope (2026-04-21)**: invalid proposal을 dispute/blacklist/retire 할 수 있거나 service를 manual restart with different configurations 할 수 있으면 otherwise valid report도 downgrade될 수 있다고 적는다. 이것은 recoverability가 이미 triage economics에 직접 들어와 있음을 보여준다.
- **Kelp / LayerZero blame dispute (CoinDesk 2026-04-20)**: Kelp는 LayerZero default path와 provider-operated verifier infrastructure를 원인으로 지목했고, 동시에 emergency pause가 후속 약 $200M 추가 유출을 막았다고 말했다. 이 조합은 팀이 upstream control-plane risk를 받아들이면서도, 나중에 pause가 있으니 감내 가능하다고 느끼기 쉬운 구조를 드러낸다.
- **Vercel / Context.ai breach (CoinDesk 2026-04-20)**: frontend/deployment plane credential 노출 가능성에 대한 대응이 곧 rotation과 inspection으로 옮겨갔다. 실제 손실이 확정되지 않았다는 사실은 중요하지만, 그 때문에 support surface의 privilege adjacency가 낮은 구조 리스크로 오인되기도 쉽다.
- **Arbitrum freeze + follow-up debate (CoinDesk 2026-04-21 / 2026-04-23)**: Security Council 개입은 일부 자금 회수와 containment에는 성공했지만, 동시에 governance-discretion과 privileged override를 노출했다. 그런데 이런 사후 회수 성공은 종종 ex-ante threat severity를 낮게 다시 쓰는 근거로 소비된다.
- **Chainalysis Kelp post-mortem (fetched 2026-04-25)**: 온체인 calldata는 정상처럼 보였고, 실제 손실 상한은 pause와 downstream freeze에 의해 바뀌었다. 중요한 점은 회수 수단이 존재했다는 사실이 아니라, 그 사실이 다음 threat-model iteration에서 off-chain verifier/RPC compromise를 덜 긴급하게 취급하게 만들 수 있다는 점이다.
- **Foundry / FV / invariant checks**: 최근 공개 신호는 nominal-path correctness를 더 잘 다루지만, severity triage가 recoverability에 의해 왜곡되는 조직 인센티브까지는 다루지 않는다. 그래서 도구가 좋아질수록 오히려 사람은 residual control-plane risk를 backstop으로 할인하기 쉽다.

**왜 퍼플팀 관점에서 새 메타 패턴인가**:

| 기존 분류 | 설명하는 것 | META-60가 추가로 설명하는 것 |
|---|---|---|
| META-58 DSCRG | default/scope carve-out ownership 부재 | ownership이 불분명한 경계를 **왜 조직이 계속 낮은 우선순위로 둬도 된다고 느끼는가** 를 설명한다 |
| META-59 NPEAA | exception path 자체의 assurance 부족 | 예외 경로의 존재가 오히려 **예방 투자와 severity 평가를 깎는 인센티브** 로 작동하는 구조를 다룬다 |
| B45 / D26 / D27 / B15 | audit attestation, frontend, RPC, key-path의 구체 벡터 | 이 벡터들이 실제 blast radius보다 **낮게 triage되는 이유** 를 설명한다 |

**왜 감사가 놓치는가**:

1. **recoverability-as-mitigation bias**: 회수 가능성을 exploitability 감소와 동일시한다.
2. **severity economics distortion**: bug bounty와 competition이 recoverability를 payout downgrade 신호로 쓰면서 연구자와 팀의 주의를 code-only path로 몰아준다.
3. **hindsight recovery halo**: 사후 동결/회수 성공이 예방 실패를 가리고, 다음 설계 주기에서 같은 경계를 다시 과소평가하게 만든다.
4. **control-plane discounting**: support surface, deployment infra, verifier/RPC plane처럼 직접 custody가 아닌 표면을 "털려도 나중에 막을 수 있다" 는 이유로 낮게 본다.

**Microstable architecture implication**:

Microstable은 이미 `emergency_shutdown`, degraded safe mode, `manual oracle mode` 같은 backstop을 갖고 있다. 이것 자체는 장점이다. 다만 퍼플팀 관점의 경고는 다음이다.

- dashboard / deploy plane / keeper key path / RPC trust 문제가 생겨도 `pause` 나 `manual oracle mode` 가 있으니 severity를 낮게 볼 유인이 생긴다.
- Blue v14/v15가 degraded write 차단과 auto emergency shutdown을 강화했기 때문에, 오히려 팀이 "최악은 막을 수 있다" 고 느끼며 support/control-plane hygiene 투자를 늦출 수 있다.
- 그러나 pause signer unavailable, recovery ceremony 지연, reputational run, oracle/manual mode drift 같은 요소가 겹치면, recoverability는 **사후 완화 수단일 뿐 사전 예방 통제의 대체재가 아니다**.

따라서 아키텍처 수준 권고는 단순하다. **severity는 먼저 backstop이 전혀 없다고 가정한 raw blast radius로 계산하고, recoverability는 그 다음 별도 항목으로만 적는다.**

**Minimum defense delta**:

1. severity triage rubric에 `recoverability discount 금지` 원칙 추가: raw blast radius 먼저, freeze/restart/rotation은 후순위 메모로만 기록
2. dashboard / deploy / keeper / verifier / RPC 경계는 "직접 custody가 아니어도 privileged control plane" 으로 분류
3. `pause signer unavailable`, `manual oracle rollback 지연`, `recovery fund 미가동` 시나리오를 포함한 tabletop 추가
4. incident report에서 `recovered amount` 와 `prevented by backstop` 을 **예방 통제 효과** 와 분리해 기록
5. bug bounty / internal review severity calibration에서 manual recovery 가능성을 direct downgrade 근거로 쓰지 말 것

**Purple Team verdict**: 많은 조직은 예외 경로의 존재를 안전망으로 삼는 데서 멈추지 않고, 그 존재를 근거로 애초의 위협을 더 작게 계산한다. META-60은 이 구조, 즉 **회수 가능성을 담보처럼 잡고 예방 투자를 깎는 보안 판단 왜곡** 을 **Recoverability-Collateralized Security Gap** 으로 고정한다.
...
- **QuillAudits KelpDAO explainer (2026-04-28 fetched)**: 공개된 재구성에서도 contract와 DVN multisig 자체는 nominal-path대로 동작했고, 실제 실패는 **DVN이 보는 RPC truth plane** 에 있었다. 권고안 역시 더 많은 contract testing이 아니라 **independent verifier domains + source-state cross-check** 로 향한다. 즉 부족했던 것은 코드 correctness가 아니라 adjacent-plane assurance였다.
- **OpenSourceMalware Vercel April 2026 IR playbook (2026-04-28 fetched)**: 실무 대응 가이드는 env-var rotation, OAuth scope inventory, third-party integration triage를 가장 먼저 둔다. 반면 Nomos/Foundry 계열 공개 guidance는 계속 nominal-path correctness를 강화한다. 이 분리는 **코어 검증이 강해질수록 대응 무게가 주변 privileged plane으로 이동한다** 는 assurance halo 문제를 다시 확인시킨다.
...
**Published**: 2026-04-29 | **Source**: Purple Team Daily Evolution | **Signals**: OWASP `Incident Response Playbook` (2026-04-28 fetched), OpenSourceMalware `vercel-april2026-incident-response` (last updated 2026-04-20), Chainalysis `Inside the KelpDAO Bridge Exploit` (2026-04-25 fetched, incident 2026-04-18), Google Cloud `Next '26: Redefining security for the AI era with Google Cloud and Wiz` (2026-04-22), Nomos Labs `Smart Contract Testing Guide: Tools and Tips for 2026`, arXiv `2604.18395` (FAUDITOR, submitted 2026-04-20), arXiv `2604.13611` (V2E, updated 2026-04-20)

**Mechanism**: 방어 실패는 더 이상 항상 “취약점을 몰랐다”에서 시작하지 않는다. 종종 팀은 이미 이상 신호를 봤다. 문제는 그 다음이다. **증거가 완전해질 때까지 기다리는 문화** 가 containment를 늦춘다.

- OWASP playbook은 severity별 response time과 containment 순서를 명시한다. 즉 사고 대응은 root-cause writeup보다 먼저 움직여야 한다.
- Vercel IR playbook은 더 노골적이다. `Rotate first, then investigate.` audit log retention이 짧고 UI가 불완전하므로, 깔끔한 "영향받음/안받음" 신호를 기다리면 이미 늦는다.
- Chainalysis의 Kelp 분석도 같은 결론이다. 첫 release는 되돌릴 수 없었지만, anomaly 인지 직후 pause가 걸리며 추가 ~$95M 유출이 막혔다. 핵심은 **첫 증명 완성** 이 아니라 **두 번째 손실 이전의 첫 조치** 였다.
- Google Cloud는 attacker hand-off가 8시간에서 22초로 줄었다고 적는다. 공격 tempo가 이렇게 짧아지면, 인간이 확증 보고서를 기다리는 문화는 구조적으로 진다.
- Nomos, FAUDITOR, V2E는 코드 correctness와 exploit validation을 더 잘하게 만든다. 유효하다. 그러나 이 도구들이 좋아질수록 팀은 오히려 "더 잘 증명한 뒤 움직여야 한다" 는 확증 편향에 빠질 수 있다. adjacent-plane 사고는 그 속도를 허락하지 않는다.

**Cross-source synthesis (최근 신호의 공통점)**:

- **OWASP IR playbook** 은 severe incident에서 response time budget을 먼저 고정한다. 즉 certainty보다 clock이 먼저다.
- **OpenSourceMalware Vercel playbook** 은 freeze deploy, disable GitHub app, snapshot audit logs, rotate secrets를 조사보다 앞에 둔다. 로그 보존이 짧기 때문에 `조사 후 대응` 이 아니라 `대응하면서 조사` 가 기본값이 된다.
- **Chainalysis / KelpDAO** 는 cross-chain invariant alert가 첫 theft를 undo하지 못해도, 두 번째 packet 전에 pause를 쏘는 것만으로 피해 곡선을 바꿀 수 있음을 보여준다.
- **Google Cloud / M-Trends signal** 은 공격자 협업과 hand-off가 인간 승인 latency를 압도하고 있음을 보여준다.
- **Nomos / FAUDITOR / V2E** 는 탐지와 검증 품질을 올리지만, 그 성공이 incident-time certainty threshold 문제까지 해결해주지는 않는다.

**왜 퍼플팀 관점에서 새 메타 패턴인가**:

| 기존 분류 | 설명하는 것 | META-62가 추가로 설명하는 것 |
|---|---|---|
| META-53 RABG | actuator가 실제로 발사 가능한가 | **언제** incomplete evidence만으로 그 actuator를 발사할지 다룬다 |
| META-55 DCREG | 선언된 제약이 집행 단계에서 힌트로 강등되는가 | 제약이 살아 있어도, **확증을 기다리느라 행동 임계치를 못 넘는 문제** 는 남는다 |
| META-61 AHTG | 코어 assurance가 주변 plane까지 번지는가 | 주변 plane이 중요하다는 걸 알아도, **확실성 부족 때문에 containment가 지연되는 구조** 는 별도 문제다 |
| META-60 RCSG | recoverability가 severity를 왜곡하는가 | recoverability bias와 무관하게, **확실성 요구 자체가 초기 대응을 지연** 시킬 수 있다 |

**왜 감사가 놓치는가**:

1. **forensics-after-correctness bias**: audit/FV/fuzz는 무엇이 exploit인지 잘 설명하지만, partial signal만으로 어떤 대응을 발사할지까지는 거의 다루지 않는다.
2. **retention is treated as ops**: audit log exportability, retention window, third-party SaaS telemetry 접근성은 보안 경계가 아니라 운영 편의/비용 문제로 분류된다.
3. **affected/not-affected fallacy**: 팀은 "영향 범위를 정확히 알기 전엔 못 움직인다" 고 느끼지만, 실제로는 그 전에 끊어야 할 secret/integration이 이미 명확한 경우가 많다.
4. **tempo asymmetry**: 공격자 hand-off와 privilege pivot는 점점 빨라지는데, defender는 여전히 확증 보고서와 승인 체인을 기다린다.

**Microstable architecture implication**:

Microstable에 오늘 새 코드 버그가 생긴 것은 아니다. 문제는 **어떤 불완전한 신호가 보이면 언제 pause / failover / signer rotation / deploy freeze를 발사할지** 의 경계가 아직 구조적으로 문서화됐는가다.

- 현재 carry-forward인 **D27**(RPC truth divergence), **A115**(dependency-latent TLS trust drift), **A75**(manual oracle fallback semantic gap), **B45**(audit attestation continuity gap) 는 모두 `완전한 root cause` 가 나오기 전에 조치해야 할 수 있는 클래스다.
- 그런데 공개 artifact 기준으로는 dashboard / RPC / build / deploy plane에 대해 **uncertainty-triggered action threshold** 가 하나의 explicit rubric으로 보이지 않는다.
- 따라서 오늘 질문은 "취약점이 확정됐는가" 가 아니라, **"확정 전에도 발사할 containment가 사전에 결박돼 있는가"** 다.

**Minimum defense delta**:

1. `RPC quorum divergence`, `unexpected artifact hash drift`, `attestation missing`, `support-plane OAuth anomaly` 같은 사건에 대해 **root cause 확정 전 containment threshold** 를 문서화
2. 조사보다 먼저 실행할 수 있는 **freeze / revoke / rotate / failover** command artifact를 별도 보관
3. control-plane audit/log evidence는 retention 짧음을 가정하고 **snapshot/export-first** 절차를 runbook에 기본 포함
4. incident review에서 `we waited for confirmation` 을 실패 원인 후보로 명시적으로 기록
5. B45를 단순 문서 누락이 아니라, **containment 판단에 필요한 continuity evidence 부족** 문제로 연결해 추적

**Purple Team verdict**: 최신 사고에서 방어는 종종 `무엇이 사실인지` 보다 `언제 충분히 위험하다고 판단하고 먼저 끊을 것인지` 에서 갈린다. META-62는 이 구조, 즉 **완전한 확증을 기다리는 문화가 containment를 구조적으로 늦추는 문제** 를 **Certainty-Seeking Containment Gap** 으로 고정한다.

**Matrix state as of 2026-04-29 (purple daily update)**: **127+ named vectors + META-01~62 + B73~B78 = 189+ total entries**. META-62 added by Purple Team 2026-04-29: **Certainty-Seeking Containment Gap (CSCG)**. OWASP IR SLA, Vercel rotate-first guidance, Kelp second-loss prevention, Google Cloud 22-second hand-off signal을 하나의 "certainty waits, attackers do not" 패턴으로 상위 구조화한다.

### META-63. Invariant-to-Operations Promotion Gap (IOPG)

**Published**: 2026-04-30 | **Source**: Purple Team Daily Evolution | **Signals**: Foundry `v1.7.0` release (2026-04-28), Chainalysis `Inside the KelpDAO Bridge Exploit` (2026-04-25 fetched, incident 2026-04-18), OWASP `Incident Response Playbook` (2026-04-28 fetched), Immunefi `Base <> Immunefi Audit Competition` (2026-04-21), Nomos Labs `Smart Contract Testing Guide: Tools and Tips for 2026`, arXiv `2604.18395` (FAUDITOR, submitted 2026-04-20), arXiv `2604.13611` (V2E, updated 2026-04-20)

**Mechanism**: 방어 실패는 더 이상 항상 `불변식을 몰랐다` 에서 시작하지 않는다. 오히려 2026년 신호는 반대다. 팀은 불변식을 더 많이 알고, 더 빠르게 찾고, 더 정교하게 검증한다. 문제는 그 다음이다. **알고 있는 불변식이 운영 신호로 승격되지 않으면**, 공격은 테스트와 감사의 밖이 아니라 **테스트와 감사의 이후** 에서 이긴다.

- Foundry v1.7.0은 invariant testing, optimization mode, time-based fuzzing을 크게 강화했다. 즉 pre-deploy 속성 탐색 능력은 빠르게 좋아지고 있다.
- FAUDITOR와 V2E는 exploitable property discovery와 PoC validation을 자동화한다. 즉 연구계도 `무엇이 exploitable한가` 를 더 잘 답한다.
- Immunefi/Base audit competition은 pre-mainnet reviewer density를 높인다. 즉 배포 전 scrutiny는 더 많아지고 있다.
- 그러나 Chainalysis의 KelpDAO 분석은 decisive control이 smart contract re-check가 아니라 **live cross-chain conservation monitoring** 이었음을 보여준다. 모든 on-chain calldata가 정상처럼 보여도, `released_on_dest <= burned_on_source` 를 운영 중 계속 확인하지 않으면 사고는 지나간 뒤에만 설명된다.
- OWASP playbook 역시 incident workflow를 자세히 적지만, 그 전제는 `어떤 속성이 깨졌는지 알려주는 신호` 가 이미 운영에 존재한다는 것이다. 이 승격이 없으면 runbook도 너무 늦게 시작된다.

**Cross-source synthesis (최근 신호의 공통점)**:

- **Foundry / Nomos / FAUDITOR / V2E** 는 더 좋은 속성 탐색과 exploit 검증을 준다. 즉 `property discovery` 층은 강해진다.
- **Immunefi/Base competition** 은 더 많은 사람이 배포 전 같은 코드를 본다. 즉 `pre-deploy review density` 가 강해진다.
- **Chainalysis / KelpDAO** 는 실제 손실 방지는 `live invariant observation` 에 달려 있음을 보여준다. 즉 `runtime promotion` 이 비어 있으면 앞단이 강해져도 decisive gap은 남는다.
- **OWASP IR playbook** 은 대응 절차를 세분화하지만, 어떤 신호를 business invariant breach로 간주할지까지는 각 팀이 운영에 박아 넣어야 한다.

**왜 퍼플팀 관점에서 새 메타 패턴인가**:

| 기존 분류 | 설명하는 것 | META-63이 추가로 설명하는 것 |
|---|---|---|
| META-15 Live-Blockchain Integration Test Gap | 테스트가 실체인 의미론을 검증하는가 | 테스트가 옳아도, **그 속성이 운영 monitor로 승격되지 않으면** 런타임에서는 여전히 침묵할 수 있다 |
| META-53 RABG | actuator가 실제로 발사 가능한가 | actuator가 있어도, **어떤 속성 위반이 그 actuator를 트리거하는지** 가 운영에 안 박혀 있으면 늦다 |
| META-62 CSCG | 언제 incomplete evidence만으로 끊을 것인가 | 그보다 앞선 문제, 즉 **무엇을 evidence로 볼지 자체를 운영 신호로 만들었는가** 를 다룬다 |
| META-61 AHTG | 코어 assurance가 주변 plane까지 번지는가 | 주변 plane이 중요하다는 걸 알아도, **그 plane의 속성을 live monitor로 승격하지 않으면** 실전 방어는 비어 있다 |

**왜 감사가 놓치는가**:

1. **artifact-ends-here bias**: audit report, formal proof, fuzz property, bounty finding이 나오면 그 자체로 안전성이 전달됐다고 느끼고, observability spec까지 이어 붙이지 않는다.
2. **ownership split**: invariant를 정의한 사람과 monitor를 운영하는 사람이 다르다. property는 문서에 남고, telemetry는 일반적인 CPU/RPC/error-rate 수준에 머문다.
3. **runtime semantics are downgraded to ops**: cross-chain conservation, artifact continuity, verifier disagreement, oracle-source divergence 같은 속성은 protocol security가 아니라 monitoring enhancement로 축소된다.
4. **tool-success substitution**: fuzz/FV/PoC validation 지표가 좋아질수록 팀은 `우리가 이 속성을 이미 관리 중` 이라고 느끼지만, 실제로는 production signal이 전혀 없을 수 있다.

**Microstable architecture implication**:

Microstable에 오늘 새 코드 버그가 생긴 것은 아니다. 문제는 **이미 알고 있는 핵심 보안 속성이 운영에 박혀 있는가** 다.

- Blue v14/v15는 mint/redeem flow cap, oracle staleness/confidence, degraded write suppression, emergency path 품질을 실제로 끌어올렸다.
- 그러나 현재 공개 artifact 기준으로는 이 속성들이 `누가`, `어떤 metric으로`, `어떤 disagreement에서`, `어떤 halt/failover로` 이어지는지를 한 장에서 묶는 **explicit invariant manifest** 가 약하다.
- 따라서 carry-forward인 **B45**(audit attestation continuity), **D27**(RPC truth divergence), **A115**(dependency-latent TLS trust drift), **A75**(manual oracle fallback semantic gap) 는 모두 `assurance exists, runtime promotion unclear` 라는 같은 메타 관점으로 다시 묶인다.

**Minimum defense delta**:

1. 핵심 보안 속성마다 `invariant → metric → alert owner → actuator` 를 잇는 manifest를 별도 작성
2. 테스트 전용 속성과 운영 필수 속성을 분리해, 후자는 dashboard/keeper/ops runbook에 기본 노출
3. `RPC quorum divergence`, `artifact hash drift`, `attestation absent`, `manual oracle mode overrun` 을 단순 운영 경고가 아니라 **security invariant breach** 로 분류
4. tabletop에서 `audit passed, monitor silent, invariant broken` 시나리오를 별도 수행
5. B45를 단순 문서 누락이 아니라, **검증 산출물이 배포·운영 연속성 신호로 승격되지 않은 사례** 로 추적

**Purple Team verdict**: 2026년의 보안 격차는 `속성을 더 잘 찾는가` 에서만 벌어지지 않는다. 진짜 차이는 **찾은 속성을 운영의 눈과 손으로 승격했는가** 에서 갈린다. META-63은 이 구조, 즉 **assurance artifact가 runtime monitor와 actuator trigger로 승격되지 않는 문제** 를 **Invariant-to-Operations Promotion Gap** 으로 고정한다.

**Matrix state as of 2026-04-30 (purple daily update)**: **127+ named vectors + META-01~63 + B73~B78 = 190+ total entries**. META-63 added by Purple Team 2026-04-30: **Invariant-to-Operations Promotion Gap (IOPG)**. Foundry/FAUDITOR/V2E의 pre-deploy 속성 강화, Immunefi/Base의 review density 증가, KelpDAO의 live conservation monitoring 필요를 하나의 `property discovery ≠ runtime protection` 패턴으로 상위 구조화한다.

### META-64. Revocation-Surface Completeness Gap (RSCG)

**Published**: 2026-05-01 | **Source**: Purple Team Daily Evolution | **Signals**: Chainalysis `The Resolv Hack: How One Compromised Key Printed $23 Million` (2026-04-30 fetched), Chainalysis `Inside the KelpDAO Bridge Exploit` (2026-04-25 fetched, incident 2026-04-18), OWASP `Incident Response Playbook` (2026-04-28 fetched), Foundry `v1.7.0` release (2026-04-28)

**Mechanism**: 많은 팀은 incident containment를 `pause 했다`, `키를 돌렸다`, `승인을 revoke 했다` 처럼 **동사 한 개** 로 생각한다. 하지만 실제 권한은 동사가 아니라 **표면 집합(surface set)** 으로 존재한다. 하나의 admin power, signer power, deploy power, oracle-override power는 보통 raw key 하나로 끝나지 않는다. KMS role, session token, GitHub/Vercel integration, pending rotation set, sidecar attestation file, downstream approval, legacy package, alternate RPC trust root처럼 **동등 권한을 운반하는 sibling surface** 가 남아 있으면 containment는 닫히지 않는다.

- Resolv는 이를 날카롭게 보여준다. 겉으로는 `SERVICE_ROLE key compromise` 처럼 보이지만, 실제 decisive boundary는 단일 비밀문자열보다 **AWS KMS-backed signing environment 전체** 였다. 문제는 키를 되찾는 것이 아니라, 같은 mint authority를 이어 주는 모든 경로를 닫는 것이다.
- KelpDAO도 마찬가지다. decisive response는 contract 한 군데를 만지는 것이 아니라 **Ethereum/L2 pause + blacklist + downstream freeze** 의 결합이었다. 즉 containment는 단일 스위치보다 **권한 그래프 절단(cut-set)** 에 가깝다.
- OWASP IR playbook은 malicious skill response에서 removal, credential reset, cache cleanup, user notification, IOC scoping을 모두 요구한다. 이것은 incident response가 본질적으로 **inventory discipline** 이라는 뜻이다. 무엇이 영향받았는지, 무엇이 같은 권한을 갖는지 모르면 runbook는 절반만 실행된다.
- Foundry v1.7.0 같은 최신 testing tooling은 pre-deploy property discovery를 강화하지만, `만약 이 plane이 compromised면 정확히 무엇을 revoke/rotate/freeze 해야 하는가` 의 **권한 인벤토리** 는 자동으로 주지 않는다.

**Cross-source synthesis (최근 신호의 공통점)**:

- **Resolv** 는 signer compromise를 raw key event가 아니라 **signing environment compromise** 로 봐야 함을 보여준다.
- **KelpDAO** 는 cross-chain containment가 single action이 아니라 **multi-surface cut-set execution** 임을 보여준다.
- **OWASP playbook** 은 containment verb보다 먼저 **affected asset / credential / installation inventory** 가 필요함을 보여준다.
- **Foundry/tooling trend** 는 발견과 검증은 빨라져도, **revocation-set completeness** 는 별도 discipline임을 보여준다.

**왜 퍼플팀 관점에서 새 메타 패턴인가**:

| 기존 분류 | 설명하는 것 | META-64가 추가로 설명하는 것 |
|---|---|---|
| META-54 DREAG | 어떤 표면이 예상 밖의 실효 권한을 갖는가 | 그걸 안 뒤에도, **동등 권한 sibling surface를 다 세지 못하면 containment는 새어 나간다** |
| META-58 DSCRG | 기본 경계와 scope carve-out을 누가 소유하는가 | ownership이 정해져도, **incident 시 revoke/rotate 대상 전체 목록이 없으면 closure가 안 된다** |
| META-62 CSCG | 언제 incomplete evidence로 끊을 것인가 | 끊기로 한 다음, **무엇을 함께 끊어야 닫히는가** 는 별도 문제다 |
| META-63 IOPG | 어떤 속성을 운영 신호로 승격할 것인가 | 신호가 울린 뒤에도, **권한 그래프 inventory가 없으면 actuator 실행이 불완전** 해질 수 있다 |

**왜 감사가 놓치는가**:

1. **principal-list compression**: authority graph를 사람/키/역할 몇 개의 목록으로 압축해, transitive sibling surface를 누락한다.
2. **exploit-path fixation**: drain path를 재현하는 데 집중하고, incident 후 닫아야 할 equivalent authority set 전체는 별도 산출물로 남기지 않는다.
3. **runbook action bias**: pause/rotate/revoke verb는 잘 적히지만, 그 verb가 덮어야 하는 asset inventory는 덜 구조화된다.
4. **false closure after partial success**: 일부 secret rotation이나 한 체인 pause 성공이 곧 containment 완료처럼 보인다.

**Microstable architecture implication**:

Microstable은 today active exploit이 새로 열린 것은 아니다. 문제는 **incident가 났을 때 무엇을 전부 닫아야 하는지 한 장에서 보이느냐** 다.

- `docs/ops-runbook.md` 는 emergency shutdown, key rotation, quorum 유지 절차를 갖고 있다.
- `solana/programs/microstable/src/lib.rs` 는 keeper 2-of-3, keeper rotation timelock, manual oracle mode, emergency shutdown을 구현한다.
- `solana/keeper/src/utils.rs` 는 expected upgrade authority, Cargo.lock / binary attestation 같은 일부 continuity check를 갖는다.
- 그러나 현재 공개 artifact 기준으로는 **keeper current/next set, expected upgrade authority, RPC/provider ownership, attestation artifact, deploy freeze 대상** 을 하나의 authority inventory로 묶은 명시적 manifest가 약하다.
- 따라서 carry-forward인 **B45**(audit attestation continuity), **D27**(RPC truth divergence), **A115**(dependency-latent TLS trust drift), **A75**(manual oracle fallback semantic gap) 는 모두 `partial containment can leave equivalent authority alive` 라는 같은 메타 관점으로 재묶인다.

**Minimum defense delta**:

1. plane별 `compromise trigger → revoke/rotate/freeze set → owner → verification evidence` 표를 만든다.
2. keeper current set뿐 아니라 **next rotation set / upgrade authority / attestation trust anchor / RPC provider ownership** 을 같은 authority inventory에 묶는다.
3. incident runbook에 `partial closure forbidden` 체크를 추가해, sibling surface 미폐쇄 상태를 명시적으로 막는다.
4. `security/audit-attestation.json` 부재, RPC failover trust, manual oracle override를 각각 독립 이슈로만 두지 말고 **동일 closure checklist** 안에서 연동한다.
5. tabletop에서 `one key rotated, equivalent authority still live` 시나리오를 별도 수행한다.
...
**Reinforcement note (2026-06-09)**: Trail of Bits의 `scan passed` scope mismatch, Foundry `#14437` 의 invariant completeness gap, 그리고 Morgan Lewis / SlowMist incident guidance를 함께 읽으면 오늘 신호는 더 선명해진다. 조직은 `scanner green`, `fuzzer present`, `monitor alerted` 같은 성공 신호를 쉽게 합성하지만, **검증면이 일부만 보거나 under-detect 하거나 signal은 냈지만 action semantics가 흐린 순간 시스템이 정확히 어떤 모드로 전환되는지** 는 종종 비어 있다. 그 결과 assurance plane은 fail-stop과 fail-open 사이에서 운영자 관행에 따라 흔들리고, pressure 아래서는 `일단 승인`, `일단 override`, `일단 primary endpoint trust` 같은 임시 해석이 사실상의 정책이 된다. 오늘 reinforcement의 핵심은 **assurance surface exists ≠ failure semantics are owned** 라는 META-66의 재확인이다.

**Reinforcement note (2026-06-15)**: Immunefi public metrics page가 **resolved reports를 2주 지연해 공개** 한다는 사실은, `bounty live`, `metrics updating`, `public leaderboard exists` 같은 신호가 곧 **실시간 assurance coverage** 가 아님을 드러낸다. 여기에 Trail of Bits의 scanner bypass 사례를 겹쳐 보면, 감사가 놓치는 구조는 단순 under-detection이 아니라 **coverage lag / opaque-surface miss / degraded-mode ambiguity** 다. 즉 assurance plane은 `있다` 보다 **무엇을 못 보고, 늦게 보고, 실패 시 무엇이 자동으로 닫히는가** 를 먼저 소유해야 한다.

**Reinforcement note (2026-06-18)**: Immunefi metrics page는 여전히 **resolved reports 기준 2주 지연** 을 명시하고 있고, current open signal인 Foundry **`#14437`** 는 같은 시간 예산에서 invariant engine이 **`0-3 bugs` vs Echidna `10+`** 수준으로 under-detect 할 수 있음을 공개적으로 적고 있다. 이 조합이 보여주는 퍼플팀 핵심은 `bounty exists`, `metrics updated`, `invariant runner present` 가 곧 **timely and complete assurance** 가 아니라는 점이다. 감사가 놓치는 구조는 검증면의 존재가 아니라, **coverage lag / completeness gap / override pressure** 를 한 장의 failure-semantics artifact로 관리하지 않는 데 있다.
...
**Published**: 2026-05-07 | **Source**: Purple Team Daily Evolution | **Signals**: RustSec `RUSTSEC-2026-0118` / `RUSTSEC-2026-0119` / `RUSTSEC-2026-0120` (issued 2026-05-01), Certora `Mastering Threat Modeling` (2026-05-05), Immunefi bug bounty surface (last updated 2026-05-06 16:00 UTC)

**Mechanism**: 팀은 validator, encoder, parser, attestation check, dependency verifier 같은 assurance layer를 붙일 때 보통 **정확성(correctness)** 과 **실패 의미론(failure semantics)** 을 이야기한다. 그런데 그보다 먼저 고정돼야 할 축이 하나 더 있다. 바로 **비용 상한(cost ceiling)** 이다.

- 어떤 입력 크기까지 허용하는가
- 검증 한 번에 얼마의 allocation / CPU / wall-clock budget을 쓸 수 있는가
- budget을 넘기면 무슨 evidence를 남기고 어떤 integrity-preserving fallback으로 전환하는가
- 반복 slow-path가 생길 때 operator가 우회나 disable pressure를 받지 않도록 어떤 guardrail을 둘 것인가

이 질문이 비어 있으면 assurance layer는 `판정기` 이기 전에 **resource-exhaustion actuator** 가 된다.

1. **validation-loop blowup**: RustSec `RUSTSEC-2026-0118` / `0120` 처럼 closest-encloser validation path가 root-stall / OOM kill-switch가 된다.
2. **encoder-side cost bomb**: RustSec `RUSTSEC-2026-0119` 처럼 parser/encoder도 O(n²) CPU sink가 되어 작은 입력 왜곡이 큰 운영 비용으로 증폭된다.
3. **operator-bypass drift**: 느리고 비싼 검증층은 결국 "일단 우회하고 나중에 보자" 는 운영 압박을 만든다. 이때 cost bug는 semantic bug로 재등장한다.

즉 문제는 `검증이 맞는가` 와 `검증이 실패하면 어떻게 되는가` 이전에, **검증이 감당 가능한 비용으로 실행되도록 설계됐는가** 다.

**Cross-source synthesis (최근 신호의 공통점)**:

- **RustSec `0118` / `0120`** 은 validation strictness가 그 자체로 memory-exhaustion surface가 될 수 있음을 보여준다.
- **RustSec `0119`** 는 encoder/compression path도 공격자가 work factor를 비틀 수 있는 assurance-adjacent surface임을 보여준다.
- **Certora threat modeling** 은 actor / dependency / assumption을 living document로 유지하라고 권한다. 퍼플팀 관점에서 여기엔 검증 단계별 `input bound / allocation ceiling / timeout budget` 도 포함돼야 한다.
- **Immunefi의 매일 갱신되는 bounty surface** 는 cheap-to-trigger, expensive-to-process 경계가 계속 시장에서 탐색된다는 뜻이다.

**왜 퍼플팀 관점에서 새 메타 패턴인가**:

| 기존 분류 | 설명하는 것 | META-67이 추가로 설명하는 것 |
|---|---|---|
| META-65 ACRSG | cheap search vs scarce response artifact | 그 전에 **방어 로직 자체가 cheap-to-trigger cost sink인지** 를 본다 |
| META-66 APFSG | 검증면이 실패할 때 어떤 의미론으로 전환되는가 | 그보다 앞서 **검증면이 감당 가능한 비용 한도 안에서 동작하는가** 를 본다 |
| D55 / A115 / B45류 개별 벡터 | 특정 validator / dependency / attestation 문제가 있다 | 서로 다른 표면을 **adversarial work-factor control** 이라는 하나의 구조로 묶는다 |

**왜 감사가 놓치는가**:

1. **benign-validator bias**: validator / encoder / parser를 방어막으로만 보고 공격자가 work factor를 조작할 수 있다고 잘 상상하지 않는다.
2. **correctness-over-cost bias**: soundness / completeness / failure semantics는 적어도, `input bound / allocation ceiling / timeout budget` 은 종종 성능 문제로 밀린다.
3. **threat-model truncation**: threat model이 자산·행위자·가정까지만 다루고, 검증 단계별 비용 상한은 빠뜨린다.
4. **bypass-pressure blindness**: 반복된 느린 검증이 결국 operator bypass·disable pressure로 이어지는 운영 역학을 과소평가한다.

**Microstable architecture implication**:

Microstable에 오늘 새 code exploit이 열린 것은 아니다. 다만 앞으로 이미 존재하거나 곧 늘어날 assurance surface에 대해 **cost ceiling manifest** 가 별도 산출물로 필요하다.

- 현재 keeper에는 secondary RPC degraded mode와 emergency shutdown path가 있다.
- startup에는 Cargo.lock / binary attestation continuity check도 있다.
- 그러나 현재 공개 artifact 기준으로는 **RPC divergence cross-check, attestation/digest verifier, dependency integrity check, future validator/prover sidecar** 에 대해 `최대 입력 크기 / 최대 allocation / per-check timeout / graceful abort / post-abort evidence` 를 한 묶음으로 고정한 증거가 약하다.

따라서 carry-forward인 **B45**(audit attestation continuity), **D27**(RPC truth divergence), **A115**(dependency-latent TLS trust drift), **A75**(manual oracle fallback semantic gap) 는 모두 `validation cost ceiling` 관점에서 다시 한 묶음이 된다.

**Minimum defense delta**:

1. assurance step마다 `input bound / allocation ceiling / timeout budget / abort evidence / integrity-preserving fallback` 표를 만든다.
2. validator·parser·cross-check sidecar는 correctness review와 별개로 **adversarial cost test** 를 릴리스 게이트에 넣는다.
3. repeated slow-path가 감지되면 silent bypass가 아니라 **evidence-preserving degraded mode** 로만 전환되게 한다.
4. tabletop에 `proof is valid but too expensive`, `parser never finishes`, `cross-check starves memory` 시나리오를 추가한다.

**Purple Team verdict**: 검증층은 맞게 판별하기만 해서는 부족하다. **공격자가 그 판별 비용 자체를 무기화할 수 없는가** 까지 못 박아야 비로소 방어 장치다. META-67은 이 구조, 즉 **검증층의 계산비용 상한이 보안 경계로 취급되지 않는 문제** 를 **Validation Cost-Ceiling Gap** 으로 고정한다.
...
**Reinforcement note (2026-06-11)**: SlowMist의 **Haedal Vault** 는 이 메타를 다시 날카롭게 만든다. 새 redeem path가 존재하고 팀이 migration을 인지하고 있어도, **deprecated deposit/share path가 여전히 live authority를 가지면 cross-version economic invariant는 그대로 깨질 수 있다**. 퍼플팀 관점에서 핵심은 단순 share inflation이 아니라, **review가 current path를 따라간 뒤에도 loss path는 legacy path에 남아 있을 수 있다** 는 점이다. 오늘 신호는 새 admission이 아니라, **`upgraded path exists` ≠ `old authority is dead`** 라는 META-68의 실전 강화다.

**Reinforcement note (2026-06-18)**: SlowMist의 **Thetanuts Finance** 와 **Aztec Connect** 는 이 메타를 더 직접적으로 재확인한다. 전자는 `Current products and active contracts were unaffected.` 라는 발표와 별개로 **legacy vault** 가 계속 live authority였고, 후자는 **3년 전 deprecated router** 가 팀 통제 밖 immutable surface로 남아 실제 손실을 냈다. 퍼플팀 기준 핵심은 `current safe`, `deprecated`, `no team control` 같은 문구가 retirement 증거가 아니라는 점이다. **old path hard-fail evidence** 가 없으면 그 표면은 여전히 attackable authority다.
...
**Published**: 2026-05-17 | **Source**: Purple Team Daily Evolution | **Signals**: arXiv `2605.11781` *Five Attacks on x402 Agentic Payment Protocol* (submitted 2026-05-12), SlowMist Hacked front page re-scan (2026-05-17 KST, no fresher competing meta delta)

**Core asymmetry**: 많은 팀은 `payment seen`, `confirmed`, facilitator ack, off-chain verify-pass 같은 **중간 상태** 를 서비스 제공에 충분한 신호로 본다. 하지만 provisional state는 settlement-grade authority가 아니다. 리소스가 한 번 열리고 응답이 캐시되거나 도구 실행이 시작되면, 나중 settlement failure·reorg·timeout·binding mismatch는 자연스럽게 rollback 되지 않는다.

**Why this became visible now**:
1. x402는 HTTP authorization(밀리초 단위)과 blockchain settlement(확률적 finality)를 한 프로토콜 안에 접합한다.
2. 논문은 live endpoints / SDK audit / testnet 실험을 통해 **grant-before-finality, settlement preemption, replay/idempotency collapse, header/proxy confusion, server-selection manipulation** 을 모두 재현했다.
3. 즉 문제는 단순 replay나 cache bug가 아니라, **어느 상태에서 entitlement를 열어도 되는가** 를 프로토콜이 security contract로 충분히 못 박지 않았다는 점이다.

**Meta causes**:
1. **UX-state equivalence bias**: `confirmed`, receipt, local verify-pass를 “사용자 경험상 충분”에서 “보안상 돌이킬 수 있음”으로 잘못 승격한다.
2. **domain-split auditing**: smart contract audit는 settlement validity를, backend review는 API/cache/idempotency를, infra review는 proxy/CDN을 따로 보며 **end-to-end grant threshold** 는 빈칸으로 남는다.
3. **rollback fantasy**: 팀은 dispute/refund/compensation을 나중에 붙일 수 있다고 생각하지만, once-delivered resource·automation·premium data는 자연 rollback이 어렵다.
4. **happy-path spec halo**: integration guide와 SDK는 quote→pay→grant happy path를 길게 설명하지만, finality depth, one-shot binding, irreversible grant 조건, cache hygiene는 짧거나 optional처럼 남긴다.

**Distinct from prior patterns**:
- **META-10** = 여러 프로토콜/파트너 사이 ownership diffusion.
- **META-66** = assurance plane이 fail/hang/diverge 할 때의 failure semantics.
- **B79** = x402 계열의 구체 exploit primitive.
- **META-69** = 그보다 한 단계 위에서, **조직이 왜 intermediate state를 settlement-grade entitlement threshold로 오인하는가** 를 규정한다.

**Audit consequence**: 앞으로 paid API, agentic commerce, off-chain execution credit, cross-system settlement를 붙이는 팀은 상태를 최소한 `quoted / presented / seen / locally-verified / confirmed / finalized / granted / consumed / refunded` 로 나누고, 각 상태가 **revocable인지 irreversible인지** 를 명시해야 한다. 특히 irreversible grant는 **finalized settlement + requester/resource/facilitator binding + one-shot idempotency burn + no-store cache discipline** 이후에만 허용돼야 한다.

**Microstable interpretation**:
- 현재 공개 Microstable artifact에는 x402 / HTTP 402 / paid API / facilitator settlement path가 없다.
- keeper의 `confirmed()` / `processed()` 사용은 외부 유료 서비스 entitlement가 아니라 on-chain tx 확인 및 readiness 판단에 한정된다.
- 따라서 **ACTIVE finding은 오늘 기준 없음**.
- 다만 향후 dashboard/keeper가 premium off-chain data, execution credit, facilitator-backed settlement, agent-to-agent paid tooling을 붙이면 **`confirmed()`·receipt·local verify를 곧바로 external entitlement로 승격하지 않는 규칙** 이 선행 검토 항목이 된다.

**Purple Team verdict**: x402가 드러낸 진짜 구조적 빈틈은 “결제가 있으면 서비스 열어도 된다”가 아니라, **어떤 상태가 아직 되돌릴 수 있고 어떤 상태만 irreversible entitlement를 허용하는가를 조직과 프로토콜이 함께 명시하지 않았다** 는 점이다. META-69는 이 문제를 **Provisional-State / Irreversible-Entitlement Gap** 으로 고정한다.

**Matrix state as of 2026-05-17 (purple daily update)**: **130+ named vectors + META-01~69 + B73~B79 = 199+ total entries**. META-69 added by Purple Team 2026-05-17: **Provisional-State / Irreversible-Entitlement Gap (PSIEG)**. x402의 payment-service correspondence 문제를 single exploit이 아니라 **중간 상태를 entitlement threshold로 오인하는 조직적 설계 실패** 로 상위 구조화한다.

---

### META-70. Node-Audit / Edge-Semantics Gap (NAESG)

**Published**: 2026-05-26 | **Source**: Purple Team Daily Evolution | **Signals**: Cloudflare **Project Glasswing: what Mythos showed us** (2026-05-18), GitHub **foundry-rs/foundry#14437** recent phased-plan issue, SlowMist Hacked front page incidents **Bankr** (2026-05-19), **Butter Bridge** (2026-05-20), **RetoSwap** (2026-05-20), **Mure** (2026-05-23), **WUSD.fi / GLOVE** (2026-05-25), Immunefi bug bounty metrics page (last updated 2026-05-25 16:00 UTC)

**Core asymmetry**: 감사와 hardening은 보통 contract, keeper, verifier, agent, dashboard, wallet 같은 **개별 노드** 를 중심으로 설계된다. 그러나 최신 공격은 점점 **노드 사이의 의미 변환(edge semantics)** 에 붙는다. signer source를 어디서 읽는지, retry/ACK 순서가 peer identity를 어떻게 바꾸는지, 어떤 fallback data가 privileged mutation을 열 수 있는지, 어떤 assistant output이 실제 wallet authority가 되는지 같은 **경계의 의미론** 이 직접 공격 표면이 된다.

**Why this became visible now**:
1. **Cloudflare Glasswing (2026-05-18)** 는 frontier tooling이 개별 bug 탐지뿐 아니라 exploit chain construction에 강해졌음을 보여주면서도, 동시에 **architecture/process around them needs to change** 라고 적었다. 즉 실전 보안은 node-local bug list만으로 닫히지 않는다.
2. **Foundry issue #14437** 는 invariant engine이 real DeFi benchmarks에서 Echidna/Medusa 대비 under-detect 상태이며, corpus/coverage/multi-step exploration 보강이 필요하다고 공개적으로 적었다. 이는 많이 쓰는 assurance plane도 **whole-path completeness** 에 아직 약하다는 신호다.
3. **SlowMist recent incidents** 는 모두 edge 문제였다: Bankr는 `LLM output → wallet authority`, Butter Bridge는 `retry message encoding → bridge authorization`, RetoSwap은 `out-of-order ACK → arbitrator node rewrite`, Mure는 `signer source selection → attacker-chosen verifier`, WUSD/GLOVE는 `per-address incentive assumption → EIP-7702 helper path` 로 무너졌다.
4. **Immunefi daily-updated bounty market** 는 이런 seam/edge class가 계속 경제적 보상을 받는다는 뜻이다. 즉 edge semantics는 이론적 틈이 아니라 실전 수익화 표면이다.

**Meta causes**:
1. **node-centric coverage bias**: 감사 스코프와 테스트 자산이 저장소·모듈·함수 단위로 짜여, `A → B → C` 경계 자체는 종종 아무도 ownership을 갖지 않는다.
2. **schema-without-semantics bias**: 메시지·해시·ACK·proof·fallback data가 형식적으로 잘 생겼거나 typed되어 있으면, 그 입력이 실제로 **무슨 권한 변환을 수행하는지** 를 끝까지 묻지 않는다.
3. **handoff demotion**: 팀 경계상 contract review, backend review, wallet review, AI review가 나뉘면서 handoff는 plumbing으로 밀린다. 하지만 공격자는 바로 그 handoff를 공격한다.
4. **runtime false-positive pressure**: edge cross-check는 noisy하다고 느껴 bootstrap/happy path에만 남고, 정작 incident 때 중요한 runtime edge invariant는 완화되기 쉽다.

**Distinct from prior patterns**:
- **META-61** = 한 레이어의 assurance가 다른 레이어까지 번지는 후광.
- **META-63** = 발견한 속성이 운영 신호로 승격되는가.
- **META-69** = intermediate state를 irreversible entitlement로 오인하는가.
- **A125 / A127 / B29 / A32** = 각각의 구체 edge exploit primitive.
- **META-70** = 그보다 한 단계 위에서, **왜 시스템이 node-level로는 잘 리뷰돼도 edge-level semantics는 끝까지 소유·검증되지 않는가** 를 규정한다.

**Audit consequence**: 앞으로는 `component inventory` 와 별도로 **edge inventory** 가 필요하다. 각 edge마다 최소한 `source`, `destination`, `authority gained`, `ordering assumption`, `provenance requirement`, `downgrade/fallback rule`, `runtime disagreement signal`, `operator evidence` 를 한 표로 고정해야 한다. fuzz/FV/invariant도 node correctness 외에 **cross-boundary negative paths** 를 자산으로 남겨야 한다.

**Microstable interpretation**:
- 현재 공개 Microstable artifact에는 이 패턴의 **active exploit** 은 보이지 않는다. keeper quorum, manual oracle cooldown, circuit-breaker precondition, dashboard RPC bootstrap verification 같은 기초 방어는 존재한다.
- 다만 구조적으로 중요한 edge는 이미 있다: `external validated prices → manual oracle mode → on-chain oracle update`, `secondary RPC health/degraded state → keeper read/write policy`, `dashboard RPC bootstrap → runtime endpoint trust`, `keeper quorum resolution → privileged parameter/oracle mutation`.
- 특히 `microstable/docs/app.js` 는 bootstrap에서는 `getGenesisHash` 를 다중 RPC로 교차검증하지만, runtime에서는 `shouldCrossCheck(method)` 가 사실상 `getGenesisHash` 만 엄격 교차검증한다. 이는 availability tradeoff로 이해되지만, 퍼플팀 관점에서는 **edge semantics가 bootstrap에는 명시돼도 runtime에는 부분적으로만 남는 구조** 다.
- 따라서 오늘 기준 **active code bug는 아니지만 MEDIUM latent architecture watch** 로 분류할 가치가 있다.

**Purple Team verdict**: 2026년의 많은 실패는 `취약한 컴포넌트 하나` 보다, **정상처럼 보이는 컴포넌트들 사이의 의미 변환이 누구의 책임으로도 완전히 고정되지 않은 곳** 에서 난다. META-70은 이 문제를 **Node-Audit / Edge-Semantics Gap** 으로 고정한다.

**Reinforcement note (2026-06-11)**: 새로 분리된 **A131 DTXT/USDT** 는 META-70의 또 다른 실전 증거다. token tax logic, pair reserve, router flow를 각각 따로 보면 치명적 reserve-break signature가 안 보일 수 있다. 하지만 경계에서는 **`paired asset moved` 라는 단순 상태 변화가 `this is addLiquidity` 라는 privileged intent proof로 잘못 승격** 됐다. 즉 각 노드는 locally plausible해도, **token policy ↔ AMM pair edge semantics** 는 공격자에게 steer 가능했다. 오늘 신호는 새 퍼플 번호보다, **heuristic edge semantics가 privileged branch를 여는 순간** 을 설명하는 META-70의 강화로 보는 편이 맞다.
...
**Published**: 2026-06-01 | **Source**: Purple Team Daily Evolution | **Signals**: SlowMist Hacked front page incidents **MoneyMon** (2026-05-29) and **ONTR** (2026-05-28), plus cumulative cross-read against **A123 Anchor System Program Identity Collapse / Arbitrary Executable CPI Surrogate**

**Core asymmetry**: 팀은 `address(0)`, `Pubkey::default()`, `None`, empty signer root, renounced owner, unset peer 같은 **sentinel/terminal state** 를 “권한 없음”, “종료됨”, “초기값” 으로 이해한다. 하지만 실제 인증 코드는 그 값을 종종 **권한 체계 밖의 값** 이 아니라 **그냥 비교 가능한 값** 으로 남겨 둔다. 그래서 invalid signature recovery, typed-wrapper special case, default-initialized config path가 같은 sentinel로 수렴하면, **실패 의미론(failure semantics)** 이 **성공 비교값(success target)** 으로 붕괴한다.

**Why this became visible now**:
1. **MoneyMon (2026-05-29)** 은 `recoverSigner(...) == admin` 만 비교하고 `ecrecover == address(0)` 실패값을 먼저 배제하지 않아, 공격자가 `admin = address(0)` 로 만든 뒤 invalid signature를 넣어 인증을 통과시켰다.
2. **ONTR (2026-05-28)** 는 renounced/null owner state를 non-authorizing terminal state로 닫지 않고 `owner == address(0)` 을 사실상 재획득 가능한 상태로 남겨 두어, “버려진 권한” 이 다시 privileged path가 되었다.
3. 같은 구조는 이미 로컬 매트릭스에도 있었다. **A123** 은 Anchor의 typed `Program<'info, System>` wrapper가 내부 sentinel collision 때문에 canonical system-program trust를 무너뜨릴 수 있음을 보여줬다. 즉 EVM의 `address(0)` 만의 문제가 아니라, **framework default / sentinel collision** 일반 문제다.

**Meta causes**:
1. **lifecycle/auth split**: renounce, disable, unset, default-init 을 운영·수명주기 문제로 보고 인증 의미론과 분리한다.
2. **admissibility blind spot**: 리뷰어는 “무엇과 비교하는가” 는 보지만, **그 비교 대상이 애초에 비교 가능한 값이어야 하는가** 는 덜 묻는다.
3. **negative-terminal test omission**: valid signer/owner/happy path는 테스트해도 `invalid recover -> sentinel`, `default pubkey`, `renounced owner`, `empty verifier root` 를 회귀 테스트 자산으로 잘 고정하지 않는다.
4. **framework-default camouflage**: sentinel 값이 라이브러리 special-case, config default, Option/nullable field 내부에 숨으면, app-layer 감사만으로는 “무해한 빈값이 권한 비교값으로 재사용된다” 는 점이 늦게 드러난다.

**Distinct from prior patterns**:
- **A123 / A129** = 구체 exploit primitive.
- **META-54** = declared role과 effective authority가 어긋나는 문제.
- **META-68** = 은퇴/legacy surface가 여전히 live authority를 갖는 문제.
- **META-71** = 그보다 한 단계 위에서, **왜 조직이 비활성/종료/기본 상태를 non-authorizing terminal state로 닫지 않고 여전히 admissible auth value로 남겨 두는가** 를 규정한다.

**Audit consequence**: 앞으로 privileged field마다 최소한 `live / rotated / revoked / renounced / unset / default` 상태를 별도 명세로 분리하고, **sentinel 값은 비교 전에 hard-fail** 해야 한다. 또한 signature, verifier root, module attachment, peer config, program identity, recovery lane 전부에 대해 `invalid result never equals terminal sentinel` 규칙을 회귀 테스트로 고정해야 한다.

**Microstable interpretation**:
- 현재 공개 Microstable artifact에는 EVM식 `address(0)` recovery path나 default pubkey authorization success lane은 보이지 않는다.
...
**Source signal (2026-04-13)**: Anchor upstream merged `fix(token2022): Add Missing Auth Account to invoke_signed` (PR #4324 / commit `ead011c`). The CPI helper for `group_pointer_update` previously constructed the instruction but omitted `authority` from the `invoke_signed` account-info slice.

**Key insight**: 공격자는 항상 “잘못된 권한 검증”만 노리지 않는다. **권한 갱신 경로 자체를 부분적으로 마비**시켜 stale pointer / stale membership / stale metadata 상태를 더 오래 유지시키는 것도 공격이다. Token-2022 extension control plane에서 authority meta 하나가 누락되면, 업데이트는 정상 governance intent가 있어도 실행되지 않는다. 이때 운영팀이 임시 우회로(`UncheckedAccount`, broad `remaining_accounts`, raw instruction fallback)를 넣는 순간, 원래는 단순 liveness bug였던 것이 **권한 혼동 또는 잘못된 meta forwarding** 공격면으로 바뀐다.

**Attack chain**:
1. protocol / issuer / registry가 Token-2022 extension pointer(`group_pointer`, `group_member_pointer`, 유사 metadata pointer)에 의존한다.
2. 상위 framework CPI wrapper가 required authority meta를 누락하거나 잘못 forward 한다.
3. 정식 revoke / rotate / cleanup 경로가 실패하면서 **기존 pointer state가 의도보다 오래 남는다**.
4. 운영팀은 hotfix로 raw CPI, permissive account forwarding, broad signer plumbing을 추가한다.
5. 공격자는 stale pointer 신뢰 또는 hotfix의 약한 계정 검증을 이용해 잘못된 group membership / trust classification / allowlist 상태를 지속시킨다.

**Why distinct from existing vectors**:
- **A51 / A56** = ExtraAccountMeta injection처럼 **추가 계정 주입**이 핵심.
- **A113** = 반대로 **필수 authority meta 누락**에서 시작해 control plane freeze → unsafe hotfix → authority confusion으로 이어진다.
- **A109 / A112** = tooling / config / metadata trust boundary 문제.
- **A113** = 실제 Token-2022 extension update CPI 경로의 **authority liveness + signer plumbing** 문제다.

**Defensive heuristic**:
- privileged CPI wrapper는 instruction builder의 metas와 `invoke[_signed]` account-info slice가 **동일 계정 집합**인지 golden-test로 고정할 것
- "단순히 안 돌아간다"는 이유로 `remaining_accounts` 나 raw `UncheckedAccount` 우회를 넣지 말 것
- pointer / membership / registry 계층은 stale state 허용 시간을 보안 파라미터로 다룰 것

**Microstable relevance**:
- 현재 Microstable은 Anchor `0.31.1` + SPL Token 경로를 사용하며 `token_2022_extensions` / `group_pointer` 사용 흔적은 발견되지 않았다.
- 따라서 **ACTIVE exploit path는 미확인**. 다만 향후 Token-2022 extension 기반 registry / compliance / asset grouping을 붙이면, 이 패턴은 즉시 설계 검토 항목이 된다.

| Vector | Mechanism | Impact | Microstable relevance |
|---|---|---|---|
| A113 Token-2022 Extension Authority-Meta Elision / Control-Plane Freeze | required authority meta omitted from privileged CPI wrapper → legit extension update path fails → stale pointer state persists → unsafe hotfix broadens authority surface | stale trust registry, delayed revoke/rotate, unsafe raw-CPI emergency patches, authority confusion | Current codebase uses classic SPL Token only; **NOT ACTIVE today**, but relevant to any future Token-2022 extension adoption |

**Matrix state as of 2026-04-14 (daily update)**: prior coverage retained; **A113** added from Anchor Token-2022 CPI wrapper fix pattern. No new CRITICAL/HIGH code-level finding for Microstable in this cycle.

<!-- AUTO-ADDED 2026-04-15 (Black Team Daily Evolution) — Dango insurance-fund sign error -->

## 2026-04-15 Insurance-Fund Signedness Pattern Addition

### A114. Signed-Amount Donation Polarity Inversion / Insurance-Fund Drain
...
**Why distinct from existing vectors**:
- **A10 Logic Bug** = 너무 일반적이다. A114는 **signed-value polarity inversion** 이라는 재현 가능한 code smell을 고정한다.
- **A67 / A89 donation-bypass류** = supply cap / accounting bypass가 핵심.
...
- keeper `price_feed.rs` / `oracle.rs` also serializes price and confidence as unsigned values.
- 따라서 **ACTIVE exploit path는 오늘 기준 미확인**. 다만 향후 insurance fund, perp PnL settlement, signed funding adjustments, or admin reserve top-up/offset path를 추가하면 A114는 즉시 재검토 대상이다.
...
### D49. Wasmtime Component-Model Transcoder Bomb / Pre-Sandbox Decoder Fault Chain

**Source signal (2026-04-09 sweep, re-evaluated 2026-04-15)**: RustSec `RUSTSEC-2026-0091`, `RUSTSEC-2026-0092`, `RUSTSEC-2026-0093` documented multiple Wasmtime component-model string transcoding faults: out-of-bounds write or crash, panic on misaligned UTF-16 strings, and heap OOB read during UTF-16 → latin1+utf16 transcoding.

**Key insight**: 공격면이 더 이상 guest sandbox 내부의 계산 로직에만 있지 않다. Wasmtime component model을 쓰면, guest가 host 함수에 들어오기 전 단계에서 **문자열 디코딩/트랜스코딩 경계 자체**가 실행면이 된다. 즉, "untrusted guest code는 sandbox 안에 있으니 안전하다"는 가정이 component ABI decoder에서 먼저 깨질 수 있다. 이 경우 공격자는 sandbox escape를 못 해도 충분하다. host 프로세스 panic, OOB read/write, privileged runner crash만으로도 oracle/keeper/automation 가용성을 무너뜨릴 수 있다.

**Attack chain**:
1. protocol / operator가 partially-trusted plugin, quote engine, simulation worker, AI tool wrapper를 Wasmtime component model로 같은 프로세스에 embed 한다.
2. attacker-controlled component가 WIT string / list payload에 misaligned UTF-16, boundary-crossing latin1+utf16 sequence, oversized transcoding case를 싣는다.
3. host-side component-model transcoder가 guest 함수 body에 들어가기 전에 panic / OOB read / OOB write를 일으킨다.
4. privileged keeper / deployer / signer-adjacent process가 죽거나 손상되어, 정상 방어 로직보다 먼저 가용성이 깨진다.
5. 운영팀이 incident 중 decoder path를 우회하려고 permissive fallback parser 또는 same-process retry를 붙이면 2차 공격면이 넓어진다.

**Why distinct from existing vectors**:
- **D46** = Winch / AArch64 Cranelift 같은 **compiler backend divergence / sandbox escape** 패턴.
- **D49** = component model의 **host-side ABI transcoding layer** 가 먼저 무너지는 패턴이다. guest instantiation 이후가 아니라, guest↔host 경계의 decoder가 공격면이다.
- **A112** = raw IDL metadata trust-boundary confusion 같은 tooling plane 문제.
- **D49** = runtime component ABI의 **string decoding/transcoding fault chain** 이다.

**Defensive heuristic**:
- privileged signer / keeper / deployer 프로세스 안에 untrusted Wasmtime component를 직접 embed 하지 말 것
- Wasmtime component-model 사용 시 patched version(`>=24.0.7`, `=36.0.7`, `=42.0.2`, `=43.0.1`) 외 버전 금지
- string/list-heavy guest↔host 인터페이스는 malformed UTF-16 / transcoding corpus로 별도 fuzz 할 것
- "sandbox 안이니 괜찮다"는 이유로 signer state, RPC creds, oracle creds를 같은 프로세스 메모리에 두지 말 것
- decoder fault는 integrity bug가 아니라 availability-only issue라고 축소하지 말 것 — keeper/oracle 경로에서는 crash 자체가 economic attack surface다

**Microstable relevance**:
- `microstable/solana/Cargo.lock` 에 `wasmtime` 가 없고, 현재 program / keeper 경로에서 component model / WIT / Wasm embedding 흔적을 확인하지 못했다.
- 따라서 **ACTIVE exploit path는 오늘 기준 미확인**. 다만 향후 strategy plugin, simulation sandbox, AI tool runner를 같은 privileged process에 붙이면 D49는 즉시 활성화된다.

| Vector | Mechanism | Impact | Microstable relevance |
|---|---|---|---|
| D49 Wasmtime Component-Model Transcoder Bomb / Pre-Sandbox Decoder Fault Chain | attacker-controlled component string payload triggers host-side ABI transcoder panic / OOB read-write before normal sandbox logic | signer-adjacent process crash, decoder-boundary memory fault, same-process plugin isolation collapse, keeper/oracle liveness loss | Current codebase has no `wasmtime` or component-model embedding; **NOT ACTIVE today**, but future plugin/agent sandbox work must isolate this path |

**Matrix state as of 2026-04-15 (red-team daily update)**: prior coverage retained; **D49** added from Wasmtime component-model transcoding advisories. No new CRITICAL/HIGH code-level finding for Microstable in this cycle.

## 2026-04-16 Keeper TLS Name-Constraint Pattern Addition

### A115. Constrained-CA Escape / Allowlisted Host Impersonation via rustls-webpki Name-Constraint Bugs

**Source signals (2026-04-15 sweep)**:
- RustSec `RUSTSEC-2026-0098` — URI name constraints were incorrectly accepted
- RustSec `RUSTSEC-2026-0099` — DNS permitted-subtree constraints were incorrectly accepted for wildcard certificates
- `solana-program/token` commit `4c6f8a7` (`deps: Update rustls-webpki`, 2026-04-15) confirms Solana ecosystem maintainers treated the advisory as an actionable dependency-security fix

**Key insight**: 많은 팀이 keeper/oracle client에서 `https://` + host allowlist만 있으면 outbound trust boundary가 충분하다고 본다. 하지만 TLS verifier가 **name constraints를 잘못 받아들이면**, 공격자는 config를 바꾸지 않고도 allowlisted hostname을 그대로 유지한 채 **misissued constrained CA certificate** 또는 **wildcard certificate** 로 신뢰 경계를 통과할 수 있다. 즉, “허용된 도메인으로만 붙는다”는 정책이 **인증서 namespace 검증 실패** 앞에서 무력화된다.

**Attack chain**:
1. keeper / relayer / off-chain signer가 allowlisted HTTPS endpoint(RPC, Hermes, CoinGecko, Binance 등)에 `reqwest` + `rustls` 기본 검증으로 연결한다.
2. 공격자는 원래는 허용되지 말아야 할 constrained subordinate CA 또는 wildcard certificate를 이용해 allowlisted host에 대한 misissued cert를 확보한다. (`RUSTSEC-2026-0098`, `0099`)
3. DNS poisoning, BGP/Anycast route hijack, upstream edge compromise, malicious corporate resolver, CDN edge takeover 중 하나로 트래픽을 자신의 endpoint로 유도한다.
4. keeper는 host string이 allowlist와 일치하고 TLS chain도 통과했다고 믿고 연결한다.
5. 공격자는 stale / biased / selectively failing RPC or price API responses를 제공해 keeper의 failover, circuit breaker, emergency mode, rebalance timing을 흔든다. 여러 소스 cross-check가 있더라도 **단일 host compromise만으로 availability degradation과 operator incident pressure** 를 유발할 수 있고, 다중 host compromise 시 integrity 공격으로 확장된다.

**Why distinct from existing vectors**:
- **A77** = CRL Distribution Point 처리 오류로 **revoked certificate가 계속 받아들여지는** revocation-bypass 패턴
- **A115** = 그 이전 단계인 **certificate issuance / namespace constraint validation failure** 다. revoke 이전에, constrained CA가 원래 못 가져야 할 namespace를 가져버리는 문제다.
- **A109 / A112** = tooling / metadata trust-boundary confusion
- **A115** = off-chain privileged client의 **TLS PKI trust-boundary** 자체가 깨지는 패턴이다.

**왜 감사가 놓치는가**:
1. 많은 리뷰가 `https://` 강제 + hostname allowlist까지만 확인하고, **certificate namespace 검증기(verifier) 내부의 name-constraint semantics** 는 재귀적으로 내려가지 않는다.
2. 스마트컨트랙트 감사는 온체인 권한·산술·CPI에 집중하고, keeper의 TLS PKI는 "인프라 기본값"으로 취급되어 별도 공격면으로 분리되지 않는 경우가 많다.
3. `CoinGecko ↔ Binance ↔ Hermes` 같은 다중 소스 교차검증이 있으면 팀은 integrity risk가 상쇄됐다고 느끼지만, 실제로는 **단일 소스 impersonation만으로도 availability degradation + incident-time hotfix pressure** 가 생긴다.
4. exploit precondition이 misissuance + routing/BGP/DNS + verifier bug의 결합이라 unit test / invariant / formal verification 기본 모델에서 거의 다뤄지지 않는다.
5. 결과적으로 운영팀은 "allowlisted host에 붙었고 TLS도 통과했다" 를 충분조건으로 오인하고, 그 아래 dependency drift(`rustls-webpki` 버전) 는 보안 KPI에 잘 드러나지 않는다.

**Defensive heuristic**:
- `rustls-webpki` 를 **`>= 0.103.12`** 로 올릴 것 (가능하면 취약한 0.101.x 계열도 제거)
- RPC / Hermes / price API는 hostname allowlist만 믿지 말고 **SPKI pinning 또는 custom CA bundle** 을 고려할 것
- `https://` 강제는 필요하지만 충분조건이 아니다. host allowlist를 certificate pinning 대체재로 취급하지 말 것
- outbound dependency host에 대해 ASN / IP drift / unexpected cert issuer change를 탐지할 것
- multi-source cross-validation이 있어도, 단일 source availability degradation이 incident hotfix 압력을 만들어 2차 우회면을 키운다는 점을 runbook에 반영할 것

**Microstable relevance**:
- `microstable/solana/keeper/Cargo.toml` 은 `reqwest = { version = "0.12", features = ["json", "rustls-tls"], default-features = false }` 를 사용한다.
- `microstable/solana/Cargo.lock` 에는 `rustls-webpki = "0.103.9"` 와 `"0.101.7"` 이 모두 존재한다.
- keeper는 `hermes.rs`, `price_feed.rs` 에서 `reqwest::Client::new()` 로 `https://hermes.pyth.network`, `https://api.coingecko.com`, `https://api.binance.com` 에 직접 연결한다.
- `config.rs` 는 `https://` 스킴과 RPC host allowlist는 강제하지만, certificate pinning은 하지 않는다.
- 따라서 **ACTIVE exploit precondition이 코드/의존성 차원에서 존재** 한다. 다만 가격 무결성 변조까지 가려면 다중 host compromise가 필요할 가능성이 높아, 오늘 기준 평가는 **MEDIUM (availability-first, integrity-conditional)** 이다.

| Vector | Mechanism | Impact | Microstable relevance |
|---|---|---|---|
| A115 Constrained-CA Escape / Allowlisted Host Impersonation | vulnerable TLS verifier accepts misissued constrained-CA or wildcard cert for allowlisted host → privileged client connects to attacker endpoint without config drift | RPC / oracle API impersonation, stale or biased off-chain inputs, keeper failover pressure, incident-time unsafe hotfix pressure | `reqwest` + `rustls-tls` keeper path is present today and `Cargo.lock` still includes vulnerable `rustls-webpki`; **ACTIVE LATENT exposure** until dependency upgrade / pinning |

**Matrix state as of 2026-04-16 (red-team daily update)**: prior coverage retained; **A115** added from `rustls-webpki` name-constraint advisories and Solana token repo dependency-fix signal. Microstable has **one MEDIUM active-latent keeper exposure** from outbound TLS trust boundary; no new CRITICAL/HIGH code-level on-chain exploit confirmed in this cycle.

## 2026-04-17 Anchor CPI Return-Data Source-Identity Pattern Addition

### A116. Anchor CPI Return-Data Program-ID Confusion / Spoofed View Result

**Source signals (2026-04-17 sweep)**:
- `solana-foundation/anchor` commit `f634129` (`fix(lang): validate program_id in CPI Return<T>::get() (#4411)`, 2026-04-16)
- upstream PoC included in the same commit: malicious program overwrites return data after a legitimate callee returns `10`, and old unchecked path reads spoofed `999`

**Key insight**: 많은 Solana/Anchor 팀이 CPI return-data를 "view helper" 정도로 여기고, 값의 **내용 타입** 만 맞으면 충분하다고 생각한다. 하지만 Solana return-data 버퍼는 트랜잭션 단위의 shared slot이라, caller가 기대한 callee가 값을 set한 직후라도 **다른 CPI callee가 마지막에 `set_return_data` 를 덮어쓰면** 내용은 그대로 보이되 **출처(program_id)** 가 바뀔 수 있다. 즉, `Return<T>::get()` 가 `T` deserialize만 하고 `get_return_data()` 의 `program_id` 를 검증하지 않으면, attacker는 "정상 프로그램이 반환한 값처럼 보이는 위조 결과"를 caller의 정책 판단에 주입할 수 있다.

**Attack chain**:
1. victim program / keeper-adjacent logic calls a trusted CPI and expects a typed return value (`Return<T>`, view helper, quote helper, validation helper).
2. trusted callee sets legitimate return-data.
3. later in the same instruction flow, attacker-controlled CPI callee executes and calls `sol_set_return_data` / `set_return_data` with the same serialized type shape.
4. old Anchor behavior reads the final buffer contents without checking which `program_id` authored the return-data.
5. caller accepts spoofed boolean / amount / quote / threshold result and takes the wrong privileged branch.

**Why distinct from existing vectors**:
- **A108 (Improper Instruction Introspection: Absolute vs Relative Indexing)** = instruction sysvar index semantics and CPI call-stack introspection confusion.
- **A116** = instruction ordering 문제가 아니라 **return-data provenance** 문제다. 값은 형식상 정상이어도, **누가 마지막에 썼는가** 가 검증되지 않으면 spoof가 가능하다.
- **A6 (Account Substitution, Solana)** = 계정 소유권/authority 계층.
- **A116** = 계정 substitution이 아니라 **ephemeral shared return buffer source identity confusion** 이다.
- **A112 (Anchor Raw IDL Metadata Trust-Boundary Confusion)** = metadata / tooling plane.
- **A116** = runtime CPI response plane, specifically `get_return_data()` provenance.

**왜 감사가 놓치는가**:
1. 많은 리뷰가 "trusted callee program_id로 CPI를 보냈는가" 까지만 보고, **응답 버퍼를 마지막에 쓴 주체가 같은가** 는 별도로 추적하지 않는다.
2. return-data는 state account를 남기지 않으므로, 감사 중 계정 그래프 기반 검토에서 존재감이 약하다.
3. 개발자는 typed wrapper(`Return<T>`) 를 쓰면 authenticity까지 같이 해결된다고 착각하기 쉽다.
4. 테스트가 보통 honest callee 하나만 둔 happy path라, malicious second-callee overwrite를 재현하지 않는다.

**PoC scenario (Microstable-adjacent future design)**:
```rust
// future risk pattern — do NOT implement this way
let quote: u64 = anchor_lang::prelude::Return::<u64>::get()?;
// team assumes quote came from trusted pricing helper

// attacker-controlled CPI runs later in the same flow
solana_program::program::set_return_data(&999u64.to_le_bytes());

// caller reads 999 and believes collateral value / fee / slippage limit passed
if quote >= min_safe_quote {
    // wrong privileged branch
}
```

**Defensive heuristic**:
- Anchor는 patched `Return<T>::get()` (program_id validation 포함) 경로를 사용할 것
- legacy / compatibility 때문에 `get_unchecked()` 류를 쓴다면, **반드시 expected callee program_id를 별도 검증** 할 것
- return-data를 access-control, oracle admission, slippage gate, collateral safety branch의 단독 근거로 쓰지 말 것
- CPI helper에 대해 **honest callee → malicious overwrite callee** 2-step PoC 테스트를 추가할 것
- 가능한 경우 return-data 대신 account-backed state, explicit event parsing, or signed message proof로 provenance를 강화할 것

**Microstable relevance**:
- `microstable/solana/programs/microstable/src/lib.rs` 와 `microstable/solana/keeper/src/` 에서 `get_return_data`, `set_return_data`, `Return::<T>` 사용 흔적을 찾지 못했다.
- 따라서 **ACTIVE exploit path는 오늘 기준 미확인**.
- 다만 향후 pricing helper, simulation helper, permission-check helper를 Anchor CPI return-data 기반 "view" 로 붙이면 A116은 즉시 활성화된다.
...
## 2026-04-22 Anchor Account-Parser Collision Pattern Addition

### D52. Anchor Composite Account-Group Name Collision / Instruction Parser Ambiguity Smuggle

**Source signals (2026-04-22 sweep)**:
- `solana-foundation/anchor` commit `df44381` / PR `#4401` (`Fix name collision in composite account de-duplicator`, 2026-04-21)
- upstream description: composite de-duplicator compared newly collected composite groups only against prior composites, not against top-level instruction account entries, allowing duplicate generated names or duplicate account-group definitions in final parser input

**Key insight**: 팀은 보통 "IDL/계정 메타는 프레임워크가 생성했으니 내부 일관성까지 보장된다" 고 가정한다. 하지만 이 버그는 **위조된 metadata를 주입하지 않아도**, honest-generated output 내부에서 composite account group과 vanilla instruction account가 같은 이름이나 같은 layout로 충돌해 **instruction parser / generated client가 애매한 정의를 소비하도록** 만들 수 있음을 보여준다.

**Attack chain**:
1. attacker-controlled repo change, compromised contributor, or unsafe upstream dependency introduces a composite account shape whose generated name/layout collides with a normal instruction account entry that appears later in the emitted output.
2. vulnerable de-duplicator keeps both definitions because it only checked composite-vs-composite collisions, not composite-vs-top-level instruction entries.
3. downstream parser, generated client, validator helper, or policy engine reuses the final output and resolves the duplicate name/group ambiguously (first-wins, last-wins, merge, or silent overwrite).
4. operator or integration code believes it is validating/signing one account schema, while the effective parsed structure maps to another group with different account ordering, mutability, or semantic meaning.
5. runtime program logic may stay unchanged, yet off-chain admission, signing, simulation, or policy enforcement is steered onto the wrong trust boundary.

**Why this is distinct from existing vectors**:
- **D31 / A112** = metadata source authenticity or trust-boundary confusion이다.
- **D52** = source는 honest-generated artifact일 수 있지만, **artifact 내부의 duplicate account-group identity** 가 parser ambiguity를 만든다.
- **A117** = signer privilege downgrade serialization failure다.
- **D52** = privilege bit 자체가 아니라, **instruction/account-schema selection ambiguity** 가 핵심이다.

**왜 감사가 놓치는가**:
1. 감사자는 대개 on-chain Rust logic와 IDL 존재 여부는 보지만, generated account-group namespace collision까지는 잘 보지 않는다.
2. duplicate name이 생겨도 build가 즉시 깨지지 않으면, parser consumer별 first-wins/last-wins 차이가 뒤늦게 문제를 드러낸다.
3. 팀은 "프레임워크가 만든 출력" 이라는 이유로 산출물 내부 일관성 검증을 생략하기 쉽다.
4. exploit은 런타임 코드 변조가 아니라 **client/parser/policy plane 오도** 로 나타나서, 사고 후에도 root cause를 business logic에서만 찾기 쉽다.

**PoC scenario (future Anchor parser/client migration)**:
```rust
// generated parser output contains two account groups with effectively colliding identity
// downstream tooling resolves one implicitly
let parsed = instruction_parser.parse(ix_data, emitted_accounts)?;

// operator believes `parsed.accounts.authority` came from the canonical top-level schema
// but parser actually bound the colliding composite definition
sign_or_validate(parsed)?;
```

**Defensive heuristic**:
- Anchor `#4401` fix 포함 버전으로 올릴 것
- generated IDL / parser input / account-group metadata에서 **duplicate group name, duplicate layout alias, ambiguous instruction-account mapping** 을 CI에서 실패 처리할 것
- parser consumer마다 first-wins/last-wins/overwrite semantics를 테스트로 고정할 것
- "generated by framework" 를 무오류 가정으로 삼지 말고, emitted account-group namespace diff를 릴리스 아티팩트 검증 항목에 넣을 것
- migration 시 old/new parser 결과를 같은 instruction corpus에 대해 diff 하여 account ordering, mutability, signer expectation이 바뀌지 않았는지 확인할 것

**Microstable relevance**:
- `microstable/solana/package.json` 은 `@coral-xyz/anchor = ^0.31.1` 이고, 테스트는 `target/types/microstable` + `anchor.workspace.microstable` 를 사용한다.
- 현재 레포에서는 Anchor 1.0 계열의 new instruction parser / composite account-group parser migration 흔적을 찾지 못했다.
- 따라서 **NOT ACTIVE today**.
- 다만 향후 Anchor parser/client generation migration이나 richer composite account schema 도입 시, generated artifact diff와 duplicate-group lint가 필수다.
