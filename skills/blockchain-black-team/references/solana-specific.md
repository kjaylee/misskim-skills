...
165. ☐ 이벤트에 호스트 실행을 바인딩하는 모든 구성(lifecycle 훅·크론 정의·CI 워크플로·스케줄러·에이전트 플러그인 메타데이터)은 자체적으로 서명/HMAC 무결성 게이트 — '구성 갱신 = 코드' 원칙. 미서명 탈출구는 런타임 플래그가 아니라 컴파일 타임(cfg!(debug_assertions)) 게이트로만; HMAC 비교는 상수시간(subtle/ct). 갱신이 관측 불능 시점에 발화하는지(주 에이전트가 못 보는 이벤트 바인딩)를 threat model에 명시 (B111; Microstable keeper verify_config_integrity가 모범 방어 실증, 경미 경화: config.rs:893 비상수시간 비교)
166. ☐ 사설 제출(릴레이·번들)은 '유출 불가'가 아니라 '유출 난이도 하락'일 뿐 — pending TX 릴레이 가시성(bundle-leak)과 일시적 소유권 이전 속성 세탁(authority-hop)을 전제로: (a) 고위험 리더 슬롯 회피(leader-risk 스코어링), (b) 다중 경로 제출(단일 릴레이 의존 금지), (c) 샌드위치 상관 기반 리더 행동 분석에 일시적 소유권 이전 시간창 상관 추가 (B78/A91 강화, bloXroute 2026-09)

### 2026-09-06 redteam — B112 LazyAccount post-CPI re-validation + Anchor v1.2.0 batch discipline

1. **Lazy accounts must never span a CPI unvalidated.** Deferred deserialization (`LazyAccount`) defers owner/discriminator checks to `load()`; pre-v1.2.0 Anchor does not re-run them after a CPI that may have rewritten the account (#4784 fixed 2026-09-04). On Anchor <1.2.0: eager `Account<'info, T>` or drop-and-re-fetch around CPI. Treat every account consumed after CPI like post-CPI lamports — re-validate before trusting.
2. **`_unchecked` constructors are manual re-validation debt.** `AccountLoader::new_unchecked` (#4162) bypasses owner + discriminator checks by design (migration aid). Any review hit on `_unchecked` requires an explicit proof of the compensating check at that call site. All-zero discriminator = reject (#4645), never an "uninitialized" marker to interpret.
...
### 2026-09-09 blackteam — A152/B114 Solana translations

168. **(A152) CPI·콜백 응답을 신뢰하기 전에 상대방을 인증하고 자금원을 자기 유도하라.** BSC 라우터 사고(09-07, $46K)의 Solana 번역: (a) 검증되지 않은 호출자가 프로그램의 콜백·CPI 응답 처리를 호출할 수 있으면(Anchor `UncheckedAccount`·raw `invoke`·수동 디직스트 채택 레인), 상대방 주소는 캐노니컬 유도(PDA/씨드/프로그램 ID)로 재계산해 `require_keys_eq!` — "누가 나를 불렀다"가 "진짜 상대"를 의미하지 않는다; (b) **자금 출처(payer/source account)는 절대 상대방 제공 데이터에서 받지 않는다** — 외부 `msg.sender` 파생 또는 진입 시점 결속; Microstable Pyth 레인이 정답형(`vault.pyth_feed` 핀 + feed_id + owner + discriminator + Full 검증 + 신선도); (c) 프로그램의 `transfer`/`transfer_from` 권한이 제3자 서명 권한(토큰 delegate·approvals) 위에 서 있으면 피해 상한은 프로토콜 TVL이 아니라 `Σ(사용자 잔여 승인액)`이다 — 위임·승인 잔액을 위협모델 자산으로 계상하고 spend cap/만료를 문서화하라.
169. **(B114) 검증 결과의 메모이제이션은 '전제 결합 함수'로만 키잉하라.** Liquid $316M의 교훈: 옳은 증명이라도 캐시 키가 증명 문장을 고유하게 식별하지 못하면 문맥 B에서 문맥 A의 검증이 재사용된다(자산 생성기·scriptPubKey 누락). Solana/키퍼 맥락: (a) 검증된 가격·계정 스냅샷·서명을 캐싱할 때 키에는 문장 의미를 바꾸는 모든 입력(feed 계정 주소 + feed_id + publish slot/time + 프로그램 ID + 자산/계정 주소)이 포함되어야 한다 — 심볼 문자열만으로 키잉 금지; (b) 음성 테스트 필수: 동일 아티팩트·다른 결속 문맥은 캐시 MISS여야 한다; (c) 쿼럼·멀티시그는 서명자에게 유효해 보이는 상태를 만드는 소프트웨어 계층의 버그에 대해 **0의 방어**를 제공한다 — 키퍼 쿼럼 승인(2-of-3)도 그 아래 검증 레인의 결속 무결성을 대체하지 않는다(HERMES-H1 posted/consumed 갭 = B114의 온프레미스 쌍둥이); (d) 합의-크리티컬 수정은 서술적 공개 커밋 마스터 푸시가 아니라 조정 릴리스로 — 공개 diff 자체가 5일짜리 역설계 창을 연다(Elements 실증).
...
174. ☐ **(B14 강화, 2026-09-12) 오라클 페치 파이프라인의 결정 필드는 서명 페이로드와 필드 결속 후에만 수용하라.** Hermes류 REST 게이트웨이의 `parsed` JSON(price/conf/publish_time)은 전송계 평문이지 서명 아티팩트가 아니다 — 서명 검증이 '그 결정이 서명된 것'을 증명하지 않는다(Whisper, arXiv 2609.11757). (a) merkle/VAA `message`를 디코딩해 feed_id·price·conf·publish_time 필드 동등성 게이트 통과 전 observation 승인 금지; (b) `parsed[i]`↔`updates[i]` 인덱스 페어링 금지 — 디코딩된 feed_id로 페어링(재배열 교차-피드 오귀속 차단); (c) 신선도 게이트는 서명 검증된 publish_time에만 적용 — 미결속 필드의 publish_time은 공격자 시계(stale-as-fresh); (d) 게이트웨이 URL은 빌드 고정 상수 + override 시 경보(설정 변수화 = B17 체인), TLS pinning 또는 scheme+host allowlist. Microstable 실증: hermes.rs:327 pinning 없는 reqwest, hermes.rs:361-377 인덱스 페어링, oracle.rs:297 미결속 신선도, config.rs:329 hermes_url override — MEDIUM latent(전제: MITM/설정 쓰기/오리진 오염), 온체인 불변식은 PTV2-002/003로 유지.
...
177. ☐ **(A2/A3-Nostra 쌍둥이, 2026-09-20) 담보의 차입력은 가조작 가능한 유동성 깊이에 결속하라 — 시가총액이 아니라.** Nostra: 시총 $547K NSTR이 $0.006→$49.5(8,250x) 리프레이스되는 동안 $3.5M 차입 허용(자기시총 ~6.4배). Solana 적용: (a) 담보 등록 시 오라클 소스(Pyth/어그리게이터/DEX TWAP)와 함께 **공격 비용 곡선**(피드를 움직이는 데 필요한 통과 유동성)을 산정하고 차입 한도·LTV·총 노출을 그 곡선에서 유도; (b) 오라클 소비자는 서명 검증과 별개로 **크기 검증**(절대 경계·슬롯당 이동폭·TWAP 편차) — feed valid ≠ price plausible; (c) 불변식 모니터: `Σ담보 평가액 ≤ 가조작 유동성 × 배수`. Microstable 실증 (PART B 2026-09-20, HEAD 23c0163): **DEFENDED 5중** — PRICE_MIN $0.50/PRICE_MAX $1.50 절대 경계(lib.rs:123-124), 신뢰구간 상한 일반 5%/민트 2%(lib.rs:52/82/1009), publish_time ≤60s(lib.rs:81), canonical_twap_price TWAP 우선 소비, 피드 계정·feed-id·authority 3중 핀; 담보 4개 전부 주요 스테이블코인 — 저유동 자기토큰 담보 등록 부재. LATENT: 변동자산 담보 추가 시 PRICE_MIN/MAX 담보별 파라미터화 + 노출-유동성 결속 필수(스테이블코인 전제 하드코딩 해제).

178. ☐ **(A153-Bonfire/Nimiq 쌍둥이, 2026-09-20) 신원 필드는 서명·필요-서명자 결속 없이 값 이동에 소비되면 안 된다 — SPL 위임 평면 도입 시 스탠딩 위임은 부채다.** 9월 5일 내 4실증(ether.fi solver 파라미터→ORBToken 자동 max 승인→Bonfire 라우터 transfer from→Nimiq 릴레이 from): 무인증 actor 필드가 제3자 승인/위임을 소비하는 4개 코드 형태. Solana 적용: (a) 프로그램 인자로 전달되는 owner/from 상당 계정은 `Signer` 강제 또는 `require_keys_eq!(owner, authority)` 없이 토큰 이동·위임·청구에 소비 금지; (b) SPL `approve`/`delegate` 도입 시(수수료 대납·메타플랜·인텐트) 스탠딩 위임은 **만료·한도 결속·레지스트리 스윕 대상 부채**로 취급; (c) 릴레이/스폰서 트랜잭션 설계 시 릴레이어가 공급한 신원을 서명 없이 수용하는 포워더 패턴 금지(서명은 nonce+chainid+target+calldata-hash 결속); (d) HTLC/해시락 계열(브리지 바우처)은 caller 선택 해시락의 trivial preimage(sha256(1) 등) 거부. Microstable 실증 (PART B 2026-09-20): **NOT ACTIVE** — 프로그램 remaining_accounts 0매치·approve/delegate 0매치·invoke 2지점 전부 구성된 시스템 전송(446/3002), UncheckedAccount 11지점 전부 require_keys_eq 재유도(pyth 핸들러 lib.rs:3145 라이브 재확인), keeper 스탠딩 승인 평면 부재. LATENT: SPL delegate 기반 수수료 대납·인텐트 파이프라인 도입 시 즉시 재평가.

### 2026-09-20 redteam — Anchor exit() 직렬화 ownership-gate(잔여 쓰기-평면) + cluster-URL 자격증명 레드랙션

- **Anchor c3e977052** (2026-09-16, #5011 "skip exit re-serialization when the account is no longer owned by the program"): 인스트럭션 진입 시점 검증(Anchor 제약·owner 체크)과 exit() 재직렬화 사이에 CPI 평면이 끼어들면 **소유권 재배정이 entry-time 직렬화 가정을 무효화**한다. 읽기 거울(§"CPI modifies PDA → in-memory struct stale")은 기등재 — 이번 확정은 **쓰기 거울**: (a) pre-fix 동작에서 CPI가 소유권을 바꾼 계정에 프레임워크가 프로그램 레이아웃 데이터를 재기록(교차-소유권 상태 크로버)하거나, (b) post-fix 동작에서 소유권 이전 계정의 영속화가 **묵시적으로 생략**되어 exit 쓰기에 의존하던 상태 갱신이 조용히 유실된다. 감사 프리미티브: `SystemProgram::assign`·realloc+assign 가젯·close-reopen 등 **소유권을 바꿀 수 있는 모든 CPI 경로 × exit/재직렬화 경로의 교차 행렬** — "entry-time 불변식이 exit-time까지 유효하다"는 프레임워크 암묵 가정을 감사가 명시적으로 깨야 한다. (PR 본문 미열람 — 커밋 제목·프레임워크 구조 기반 흡수, 0.31.1 내부 구현 단정은 보류)
- **Anchor 8039acf14** (2026-09-18, #5061 "redact secrets from cluster URLs printed during deploy/keys sync"): RPC 엔드포인트 URL userinfo(`https://key@rpc`) 자격증명이 deploy/keys-sync 출력에 평문 노출되던 툴체인 평면. 2026-07-04 스킴-스머글링 항목과 같은 입력의 다른 오염면(기밀성). 일반화: **운영 자산의 URL-embedded credential은 로그·에러 메시지·파노믹 익셉션·배포 출력 전 경로에서 레드랙션 대상**.
- **arXiv 09-16/17 배치**(회차 결손 09-17~19의 창 흡수): 「Competition, Collusion, and Corruption: MEV Attacks on DAG-Based BFT Consensus」(09-17) — MEV의 합의-오더링 일반화(경쟁·공모·부패 스펙트럼). Solana는 DAG-BFT 아님, Jito 오더링 레인은 기존 D27/MEV 커버 — 방법론 참고급, named 미달. 「AgentLSD」(09-16) — 보안 에이전트 대상 adversarial task contamination, B119 인접 강화(우리 keeper 결정론 AIG+tournament라 NOT ACTIVE 불변).
- **RustSec 창 내 신규 4건**: RUSTSEC-2026-0286(cryptoki)·0287/0288(cosmian_kyber)·0289/0290(pqc_kyber) — PQC Kyber AVX2 cmov가 FO implicit rejection을 건너뛰는 키 회복 계열. `microstable/solana/Cargo.lock` 4크레이트 전부 **0매치 — 무관**.
- **Anchor 창 내 기타**(09-16~09-18): #5064(AVM self-copy)·#5057(create_account_allow_prefund v1)·#5052/security.json 메타·#5010(IDL 주소 `ID` 결정론)·#4931(CPI return data Vec 저장 — return-data 평면은 ☐62 기커버, 버퍼 저장 각도만 미흡 신규성)·#4668/#4623(코드젱 경로 위생)·#4807/#4525(TS 클라)·CLI류 — 전부 툴체인/코드젠/클라이언트, 비보안 기각.
- **Microstable 실측 (2026-09-20, HEAD 23c0163)**: exit-크로버 **NOT ACTIVE(구조적)** — `assign`/`SystemProgram::assign` 코드 0매치(lib.rs:784·2912는 주석·에러 문자열), remaining_accounts 0, CPI 전수(446/3002 시스템 전송 + token transfer_checked/burn/mint_to)에 소유권 재배정 경로 부재. URL-credential **NOT ACTIVE** — keeper rpc_url 로깅 0매치, `extract_https_host()` https 정밀 매칭(2026-07-04 항목 실증 유지).

179. ☐ **(2026-09-20) CPI 평면에서 소유권을 재배정할 수 있는 모든 경로 × exit/재직렬화 경로의 교차 행렬을 감사하라.** (a) `SystemProgram::assign`·realloc-assign 가젯·close-reopen을 호출하거나 허용하는 CPI 전수 열거; (b) 사용 Anchor 버전의 exit()가 소유권 상실 계정을 재직렬화(크로버)하는지 생략(조용한 상태 유실)하는지 확인 — 어느 쪽이든 "entry-time 불변식 ≠ exit-time 상태"이며 이 간극을 명시적 불변식으로 닫을 것; (c) 방어: 치명 계정은 exit 재직렬화 암묵 의존 대신 명시적 재검증+커밋 경로로 영속화, 소유권 변경 CPI는 정책상 금지 목록화. Microstable: NOT ACTIVE(구조적 — 소유권 재배정 CPI 부재). LATENT: 소유권 이전 기반 핸드오프(에스크로 양도·프로그램 마이그레이션) 도입 시 즉시 재평가.

180. ☐ **(2026-09-20) URL-embedded 자격증명(userinfo)은 파싱 거부 + 전-로그-경로 레드랙션 대상이다.** RPC/게이트웨이/DB 엔드포인트의 `scheme://user:pass@host` 형태: (a) 설정 로더에서 userinfo 존재 자체를 거부하거나 분리-비밀 저장소로 강제; (b) 로그·에러·파노믹 익셉션·배포 출력은 host만 렌더(Anchor #5061의 자산-운영판); (c) 커밋 히스토리·CI 아티팩트·디버그 덤프에서 userinfo 패턴 정기 스윕. Microstable: NOT ACTIVE(rpc_url 로깅 0매치 + https 정밀 파싱).

### 2026-09-23 blackteam — A155(Radix borrowed-reference downgrade) Anchor 쌍둥이

- **Radix 엔진 결함의 Anchor 일반형**: 권한이 참조의 *분류*(타입·서명자 상태·소유자·PDA 유도 여부)에서 유도되고, 참조가 신뢰 경계(엔트리 컨텍스트→헬퍼 함수→CPI→콜백)를 가로지를 때 분류가 재평가되지 않으면 **대여 자체가 분류 세탁 단계**가 된다. Anchor 표면: (a) `UncheckedAccount`/`AccountInfo` 전달은 소비 프레임에서 `require_keys_eq!`/owner/discriminator 재유도 없이 소비되면 A155 — 소비 지점 재유도 교리와 정확히 일치; (b) PDA 참조는 소비 지점에서 유도 시드 재계산으로 기원 증명; (c) CPI 수신자는 "호출자가 갖고 있었으니 인가됐다"는 상속 금지 — CPI 프레임에서 서명자/소유자 독립 재확립(A70 계열과 교차). Radix 공식 포렌직의 **AI 코드분석 발견** 귀속 주목: 공개 소스의 잠복 결함이 AI 탐색 비용으로 노출됨 — 인가/발행 경로의 연속 자동 재검토가 방어의 대칭 의무가 됨.

181. ☐ **(A155 쌍둥이, 2026-09-23) 참조의 권한 분류는 모든 프레임/대여 경계에서 재유도되거나 증명 가능하게 보존되어야 한다 — 이전 프레임의 분류 상속 금지.** (a) 값 보유 객체(토큰 계정·민트·vault 상태)의 참조가 헬퍼 함수·CPI·콜백으로 전달되는 전수 경로에서, 소비 지점이 해당 프레임의 실제 기원(key 동등·owner·서명자·PDA 재유도)으로 권한을 재확립하는지 검증; (b) "엔트리에서 검증됐으므로 콜리에서 재검증 불필요" 관행은 A155 정의상 결함 — 엔트리 검증과 콜리 재검증의 비대칭이 있는 경로를 명시적 불변식으로 문서화하거나 폐쇄; (c) 프레임워크·엔진 수준 권한(Anchor 제약·SPL 소유권)에 의존하는 설정(role·allowlist)은 그 엔진 강제가 실패할 경우의 잔여 폭을 명시 — role이 "allow all"이면 엔진 소유권 강제가 유일한 방어선임을 위협모델에 기록; (d) 감사·검증 아티팩트는 commit×path에 결속되어야 하며 커버된 인가/발행 경로의 재작성은 재검토 트리거(audit-revision drift). Microstable 실증 (PART B 2026-09-23, HEAD 23c0163): **DEFENDED(소비-프레임)** — pyth `UncheckedAccount` 소비 함수 내 권한 재유도(require_keys_eq lib.rs:3159-3162 + feed_id 동등 + publish_time + PRICE_MIN/MAX + confidence + owner), 마이그레이션 UncheckedAccount 전부 바디 결속, remaining_accounts 0. LATENT: 헬퍼로 참조를 넘기는 신규 경로 도입 시 소비 지점 재유도 강제.
182. ☐ **(RUSTSEC-0285 자산화, 2026-09-23) keeper TLS 백엔드는 단일 평면이다 — 어드바이저리 추적·lockfile 패치를 CI가 소유하라.** keeper reqwest는 `rustls-tls` 단일 백엔드(`default-features = false`, native-tls 폴백 부재 — keeper/Cargo.toml:23)로 Hermes/RPC HTTPS 전 경로를 수행한다. (a) TLS 스택 크레이트(rustls·hyper-rustls·quinn-proto·ring·rustls-webpki)의 RustSec 발행을 cargo-audit/deny CI가 상시 추적 — MEDIUM 이상 발행 시 lockfile 갱신 없는 배포 금지; (b) lockfile-only 패치는 프로덕션 빌드 전 즉시 반영 — 선례: SPL token #179(09-18)이 RUSTSEC-2026-0285로 rustls 0.23.37→0.23.45를 lockfile-only로 즉시 반영, Microstable은 `cargo update -p rustls` 권고(09-15 블랙팀 배치) 8일째 미집행; (c) 어드바이저리 영향 판정은 원문 텍스트 기준으로 — 0285는 "handshake transcript is still authenticated"(I:N)로 MITM이 핸드셰이크 변조·완성 불가, 따라서 B14/RTD-2026-0912-01(Hermes 결속 갭)과의 합성 상향은 불성립: CVSS 라벨(5.3 MEDIUM)과 실질 무결성 영향(없음)을 구분해 기록할 것; (d) Microstable 실증: Cargo.lock rustls 0.23.36(영향 범위 ≥0.23.13 <0.23.45) + 0.21.12(unaffected) 이중 엔트리 — MEDIUM 캐리포워드.
183. ☐ **(C23-Neutron 번역, 2026-09-24) admin/upgrade authority를 DAO·체인 거버넌스에 위임한 배포는 위임 대상 권한 평면의 침해 비용을 자기 위협 모델에 포함해야 한다.** (a) 온체인 프로그램의 upgrade authority·admin이 SPL Governance DAO/멀티시그에 위임된 경우 해당 거버넌스의 제안 정족수·참여율·지연이 곧 프로그램 방어의 상한 — 감사는 앱 코드만 커버(audit-unit ≠ compromise-unit); (b) admin 재할당/권한 이전 명령은 timelock + 이중 서명(2-of-N)으로 게이트 — 단일 통과 제안이 권한 평면 전체를 재작성해서는 안 됨; (c) 거버넌스 제안의 설명 라벨("research"/"upgrade")은 검증이 아니라 메타데이터 — 투표자·자동화의 승인 프록시로 사용 금지(META-73 거버넌스 평면 번역); (d) Microstable 실증 (PART B 2026-09-24, HEAD 23c0163): **NOT ACTIVE(구조적)** — 권한 평면 TRUSTED_INITIALIZER `require_keys_eq` + 2-of-3 keeper_set(lib.rs:153-163), EmergencyShutdown 이중 서명(lib.rs:2602-2610), 온체인 거버넌스 평면·admin 재할당 명령 부재, `devnet_force_reinit`은 devnet-admin 피처 게이트; 메인넷 프로그램 불변성은 `security/program-immutability-proof.txt`(2026-02-22 검증)로 문서화 — 업그레이드 오소리티 폐기 상태. LATENT: 향후 DAO 위임·업그레이드 가능 배포 전환 시 제안 정족수·참여율·지연을 위협 모델에 포함 필수.
184. ☐ **(Anchor #4845 번역, 2026-09-24) `*_if_needed` 초기화 분기는 "이미 존재해서 스킵한" 계정의 Token-2022 extension 세트를 온체인 재검증하라.** (a) anchor-spl `init_mint_if_needed`/`init_token_if_needed`의 스킵 분기는 기존 계정을 프로그램 가정(보통 "확장 없음")과 동일하다고 암묵 취급한다 — 해당 계정의 extension 구성(TransferHook·TransferFee·NonTransferable·InterestBearing·MetadataPointer)을 init 직후 assert할 것(otter-sec/anchor #4845, 09-23 커밋 2bfd1fe가 상류에서 수정한 암묵 가정); (b) 선점(pre-init) 경로: mint 주소가 프로그램 PDA가 아니거나 seeds 예측 가능하면 공격자가 extension-laden mint을 선제 생성해 if_needed 스킵을 유도 — mint PDA 유도 강제, 불가 시 extension 세트 명시 검증; (c) TransferHook extension은 기존 Token-2022 CPI-callback/stale-data 벡터(matrix 해당 섹션)와 직접 결합하므로 extension 세트 검증은 그 벡터의 사전 차단 계층; (d) 한계: PR 본문 미열람 — 커밋 제목+구조 기반 흡수(#5011 관행 동일). Microstable 실증 (2026-09-24, HEAD 23c0163·mtime 02-28 동결): **NOT ACTIVE(구조적)** — `init_mint`/`InitMint`/`token_2022`/`Token2022`/`extension` 프로그램 전수 0매치, mstb_mint·담보 mint은 init 시 canonical 바인딩(FIX CR-01, lib.rs:2188). LATENT: Token-2022 전환·신규 담보 mint 추가 시 즉시 재평가.

### 2026-09-25 redteam — 보호 오더플로 잔여 샌드위치(arXiv 2609.28115) + 실패 tx 팬텀 이벤트(Anchor #4884)

- **arXiv 2609.28115 "No Place to Hide: An Analysis on Protected Order Flow Sandwich Attacks"**(09-23 리스팅, 3년 종단측정): Ethereum·**Solana**·Tron·Base·Arbitrum·Monad 6체인에서 프라이빗 RPC·네이티브 보호(protected order flow) 대상 샌드위치 **2,800만 건 식별**(초록 기준) — 블록 내·**크로스블록 wide** 공격 탐지 휴리스틱 + 봇 행동 필터링으로 합법 거래와 구분. 기존 매트릭스(A8/C25/A71/META-09·Aave/CoW 흡수)의 '보호 실패→공개 멤풀 노출' 고장 모드와 달리 **보호가 설계대로 작동해도 잔여 샌드위치가 존재함을 측정으로 실증** — 제출 레인 사유화는 경계(boundary)가 아니라 확률적 완화(mitigation).
- **Anchor #4884 "fix(client): Don't report failed transactions in events listener"**(97e2d91, 09-23 23:22 UTC, merged): pre-fix 클라이언트가 **실패 tx의 이벤트를 정상 이벤트로 보고**. Solana 런타임은 실패 tx의 상태 변경을 전량 롤백하지만 실행 중 발행된 이벤트/로그는 관측 가능 — 이벤트 소비자가 커밋되지 않은 상태 전이에 반응한다. D27(RPC 오염, 이벤트 '내용' 위조)과의 구분: 여기선 이벤트가 진짜 프로토콜 이벤트이고 오염면은 '실행 상태' 채널.

185. ☐ **(2026-09-25) 사유/보호 제출 레인(Jito·프라이빗 RPC)은 MEV 방어 '종결' 주장의 근거가 될 수 없다 — 잔여 노출을 위협모델에 명시하라.** (a) 사유 레인 채택·권고 문서에서 "제출 경로 사유화 = 샌드위치 차단" 서술 금지 — 크로스블록 wide 샌드위치·레인 유출·경매 내부 우선권(A71 Jito tip auction)은 레인 작동과 무관하게 실증된 잔여 경로; (b) 모니터링은 단일 블록/단일 베뉴 시야를 넘는 종단 wide-샌드위치 휴리스틱(블록 경계 횡단 가격 왜곡 지속 + 봇 행동 필터) 포함 — 표준 모니터는 이 클래스에 구조적으로 맹목; (c) 방어 클레임 감사: "protected flow 사용"을 컨트롤로 기록한 문서는 완화(mitigation) 항목으로 재분류. Microstable 실증 (2026-09-25, HEAD 23c0163): **NOT ACTIVE(구조적)** — keeper 표준 RPC 제출, jito/bundle/private-rpc 전수 0매치(사유 레인 방어 클레임 자체가 부재). LATENT: 프론트러닝 방어 목적 사유 레인 도입 시 '종결' 아닌 '완화'로 위협모델 기록 + wide-샌드위치 모니터링 동반 필수.
186. ☐ **(2026-09-25) 이벤트/로그 소비 지점은 트랜잭션 실행 상태(tx status)와 결속하라 — 실패 tx의 팬텀 이벤트가 오프체인 자동화를 오염시킨다.** Solana 실패 tx = 계정 상태 전량 롤백이지만 프로그램 이벤트/로그는 관측 가능(Anchor #4884가 상류에서 수정한 클라이언트 암묵 가정 "이벤트 발행 = 상태 커밋"). (a) 모든 이벤트/로그 소비 지점 전수 열거 — meta.status/err 결속 없이 이벤트를 상태기계 입력으로 소비하는 경로는 결함; (b) 상태기계는 event-without-commit을 비이벤트로 처리하고, 경보·리밸런스·지급 트리거는 커밋 확인 후에만; (c) 공격 원가가 저렴함에 주목 — 실패하는 후속 인스트럭션만 붙이면 되므로 이벤트 모방 공격에 서명·권한 불필요(공격자가 가짜 이벤트를 발행하는 게 아니라 **진짜 프로토콜 이벤트가 커밋 없이 관측되게** 만든다 — D27 내용-위조와 직교하는 상태-채널 오염); (d) Anchor 클라 체인 의존 시 #4884 수정 포함 버전으로. Microstable 실증 (2026-09-25): **NOT ACTIVE(구조적)** — keeper Anchor 클라 이벤트 리스너 소비 부재(addEventListener/onLogs/Event::decode 전수 0매치; utils.rs:651 events HashMap은 로컬 레이트리밋 상태, 온체인 소비 아님). LATENT: 이벤트 기반 상태 동기화·인덱서·얼러터 도입 시 tx-status 결속 강제.

187. ☐ **(ToB TEE+MPC 번역, 2026-09-26) 파괴적 키-상태 전이(논스·프리시그니처 소비/삭제)는 롤백-저항적 종결을 요구한다 — 호스트 제어 저장장치에서 '삭제'는 어태스테이션 없이는 되돌릴 수 있다.** Trail of Bits 「Don't let TEEs break your MPC」(2026-09-25): TEE 내 threshold signer가 사용된 pre-signature를 삭제한 뒤 **악성 호스트가 파일시스템 상태를 롤백**하면 논스 쉐어가 재사용되어 개인키 쉐어가 유출된다 — MPC의 임계값 보호가 TEE 호스트의 상태-되감기와 조합되는 지점에서 무너지는 경로. (a) 일회용 암호상태(논스·프리시그니처·시퀀스)의 소비·삭제는 단조 카운터·어태스테이션 결속 영속 계층에서만 종결로 인정; (b) 어태스테이션은 MPC 당사자 신원에 결속(ToB 권고) — 무기명 TEE 증명은 롤백 호스트를 걸러내지 못함; (c) 커스터디·서명 인프라 감사 체크리스트에 '삭제 후 호스트 롤백 재현' 시나리오 명시. Microstable 실증 (2026-09-26): **NOT ACTIVE(구조적)** — keeper 서명 키는 평문 키파일(TEE/MPC 커스터디 부재), Solana 리플레이 보호는 블록해시 기반·durable nonce는 온체인 권한 → 호스트 파일시스템 롤백으로 '소비된 논스'를 부활시킬 수 없음(ToB 패턴에 대한 구조적 방어 관찰). LATENT: 키 커스터디를 TEE/MPC 서명자로 이전하는 시점 즉시 재평가 + 일회용 상태 종결에 온체인 단조 권한(논스 어카운트·시퀀스) 강제.
