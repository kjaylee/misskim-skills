...
- **동시 에이전트 상한 5개** — 한 번에 실행되는 서브에이전트는 최대 5개. 초과 시 완료된 에이전트를 정리한 후 새 에이전트 디스패치.
- **공유 리소스 충돌 방지** — 같은 파일, DB 테이블, 포트를 여러 에이전트가 동시에 쓰는 경우 mutex 또는 파일 분리 전략 명시. 에이전트 프롬프트에 `수정 금지 파일` 목록 포함.
- **실패한 에이전트 자동 정리** — 에이전트 실패(타임아웃, 오류) 시 해당 에이전트의 임시 파일, 락 파일, 부분 커밋을 즉시 정리. 정리 완료 후 재시도 또는 에러 보고.
- **전체 파이프라인 타임아웃 설정 필수** — 개별 에이전트 타임아웃 외에 전체 병렬 파이프라인에 대한 최대 실행 시간을 설정. 기본값 600초 (개별 에이전트 300초 × 2단계). 타임아웃 초과 시 실행 중인 모든 에이전트 강제 종료 후 Master에게 보고.
